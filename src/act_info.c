...
char *format_obj_to_char( OBJ_DATA *obj, CHAR_DATA *ch, bool fShort )
{
    ...
    if (  (IS_AFFECTED(ch, AFF_DETECT_EVIL) || xIS_SET( ch->class, CLASS_PALADIN) )
...
char *hallucinated_object( int ms, bool fShort )
{
	...
	case  4: return "your bloody head on a stick";
	...
	case 15: return "One of Thoric's knocked out teeth";
	...
	case  4: return "Your own face stares back at you from the ground.";
	...
	case 15: return "You see one of of Throic's knocked out teeth here.";
...
void show_list_to_char( OBJ_DATA *list, CHAR_DATA *ch, bool fShort, bool fShowNothing )
{
	...
	* (IS_NPC(ch) ? 1 : (ch->pcdata->condition[COND_DRUNK] ? (ch->pcdata->condition[COND_DRUNK]/12) : 1));
	...
	if ( fShowNothing )
	    send_to_char( "     ", ch );
	send_to_char_color( prgpstrShow[iShow], ch );
/*	if ( IS_NPC(ch) || xIS_SET(ch->act, PLR_COMBINE) ) */
	{
	    if ( prgnShow[iShow] != 1 )
...
void show_char_to_char_0( CHAR_DATA *victim, CHAR_DATA *ch )
{
    ...
    if ( IS_EVIL(victim)
    &&   (  IS_AFFECTED(ch, AFF_DETECT_EVIL) || xIS_SET( ch->class, CLASS_PALADIN ) )     ) strcat( buf, "(Red Aura) "   );
    if ( IS_NEUTRAL(victim)
    &&   xIS_SET( ch->class,CLASS_PALADIN )  ) strcat( buf, "(Grey Aura) "   );
    if ( IS_GOOD(victim)
    &&   xIS_SET( ch->class, CLASS_PALADIN )  ) strcat( buf, "(White Aura) "   );
  ...
  if ((victim->position == victim->defposition && victim->long_descr[0] != '\0')
    ...
    case POS_SITTING:
        if (ch->position == POS_SITTING)
        {
        if (victim->sitting_on != NULL)
        {
            if (IS_SET(victim->sitting_on->value[2],SIT_AT))
            {
                act( AT_PURPLE, "$N is sitting at $p.",ch,victim->sitting_on, victim, TO_CHAR);
            }
            else if (IS_SET(victim->sitting_on->value[2],SIT_ON))
            {
 		act( AT_PURPLE, "$N is sitting on $p.",ch,victim->sitting_on, victim, TO_CHAR);
            }
            else
            {
 		act( AT_PURPLE, "$N is sitting in $p.",ch,victim->sitting_on, victim, TO_CHAR); 
	    }
        }
        else
    ...
    if(victim->position != POS_SITTING || victim->sitting_on == NULL )
    {
       strcat( buf, "\n\r" );
       buf[0] = UPPER(buf[0]);
       send_to_char( buf, ch );
...
void do_look( CHAR_DATA *ch, char *argument )
{
	...
	if( time_info.hour >= TIME_SUNRISE && time_info.hour < TIME_SUNSET) {

	  switch ( ch->in_room->sector_type ) 
	{
	   case SECT_INSIDE:
	     set_char_color( AT_RMDESC_INSIDE_DAY, ch );
	     break;
	   case SECT_CITY:
	     set_char_color( AT_RMDESC_CITY_DAY, ch );
	     break;
	   case SECT_FIELD:
	     set_char_color( AT_RMDESC_FIELD_DAY, ch );
	     break;
	   case SECT_FOREST:
	     set_char_color( AT_RMDESC_FOREST_DAY, ch );
	     break;
	   case SECT_HILLS:
	     set_char_color( AT_RMDESC_HILLS_DAY, ch );
	     break;
           case SECT_MOUNTAIN:
             set_char_color( AT_RMDESC_MOUNTAIN_DAY, ch );
             break;
           case SECT_WATER_SWIM:	
             set_char_color( AT_RMDESC_WATER_SWIM_DAY, ch );
             break;
           case SECT_WATER_NOSWIM:
             set_char_color( AT_RMDESC_WATER_NOSWIM_DAY, ch );
             break;
           case SECT_UNDERWATER:
             set_char_color( AT_RMDESC_UNDERWATER_DAY, ch );
             break;
           case SECT_AIR:
	     set_char_color( AT_RMDESC_AIR_DAY, ch );
	     break;
	   case SECT_DESERT:
	     set_char_color( AT_RMDESC_DESERT_DAY, ch );
	     break;
           case SECT_DUNNO:
             set_char_color( AT_RMDESC_DUNNO_DAY, ch );
             break;
           case SECT_OCEANFLOOR:
             set_char_color( AT_RMDESC_OCEANFLOOR_DAY, ch );
             break;
	   case SECT_UNDERGROUND:
	     set_char_color( AT_RMDESC_UNDERGROUND_DAY, ch );
	     break;
           case SECT_LAVA:
             set_char_color( AT_RMDESC_LAVA_DAY, ch );
             break;
           case SECT_SWAMP:
             set_char_color( AT_RMDESC_SWAMP_DAY, ch );
             break;
           case SECT_HIVE:
             set_char_color( AT_RMDESC_HIVE_DAY, ch );
             break;
	   default:
	     set_char_color( AT_RMDESC_DAY, ch );
	     break;
        }
	...
	{ 
	   case SECT_INSIDE:
	     set_char_color( AT_RMDESC_INSIDE_NITE, ch );
	     break;
	   case SECT_CITY:
	     set_char_color( AT_RMDESC_CITY_NITE, ch );
	     break;
	   case SECT_FIELD:
	     set_char_color( AT_RMDESC_FIELD_NITE, ch );
	     break;
	   case SECT_FOREST:
	     set_char_color( AT_RMDESC_FOREST_NITE, ch );
	     break;
	   case SECT_HILLS:
	     set_char_color( AT_RMDESC_HILLS_NITE, ch );
	     break;
	   case SECT_MOUNTAIN:
	     set_char_color( AT_RMDESC_MOUNTAIN_NITE, ch );
	     break;
           case SECT_WATER_SWIM:
             set_char_color( AT_RMDESC_WATER_SWIM_NITE, ch );
             break;
           case SECT_WATER_NOSWIM:
             set_char_color( AT_RMDESC_WATER_NOSWIM_NITE, ch );
             break;
           case SECT_UNDERWATER:
             set_char_color( AT_RMDESC_UNDERWATER_NITE, ch );
             break;
           case SECT_AIR:
             set_char_color( AT_RMDESC_AIR_NITE, ch );
             break;
	   case SECT_DESERT:
	     set_char_color( AT_RMDESC_DESERT_NITE, ch );
	     break;
           case SECT_DUNNO:
             set_char_color( AT_RMDESC_DUNNO_NITE, ch );
             break;
           case SECT_OCEANFLOOR:
             set_char_color( AT_RMDESC_OCEANFLOOR_NITE, ch );
             break;
	   case SECT_UNDERGROUND:
	     set_char_color( AT_RMDESC_UNDERGROUND_NITE, ch );
	     break;
           case SECT_LAVA:
             set_char_color( AT_RMDESC_LAVA_NITE, ch );
             break;
           case SECT_SWAMP:
             set_char_color( AT_RMDESC_SWAMP_NITE, ch );
             break;
           case SECT_HIVE:
             set_char_color( AT_RMDESC_HIVE_NITE, ch );
             break;
	   default:
	     set_char_color( AT_RMDESC_NITE, ch );
	     break;
        }


	  if ( arg1[0] == '\0' || ( !IS_NPC(ch) && !xIS_SET(ch->act, PLR_BRIEF) ) ) {
	    if( ch->in_room->nitedesc != STRALLOC( "" ) )
	      send_to_char_color( ch->in_room->nitedesc, ch );
	    else send_to_char_color( ch->in_room->description, ch );
	  }
	}
	...
	if ( !IS_SET(ch->in_room->room_flags, ROOM_NO_EXIT) && !IS_NPC(ch) && xIS_SET(ch->act, PLR_AUTOEXIT) )
	    do_exits( ch, "auto" );
	...
	if ( !CAN_WEAR( obj, ITEM_TAKE ) && ch->level[max_sec_level(ch)] < sysdata.level_getobjnotake )
	{
	...
	||   get_trust(ch) >= LEVEL_IMMORTAL )) && !IS_SET( pexit->exit_info, EX_NOSCRY))
	{
...
void show_condition( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( victim->max_hit > 0 )
        percent = victim->hit  / ( .01 *victim->max_hit ) ;
    else
        percent = -1;
...
/* start from last to first to get it in the proper order */
	...
	||   !can_see( ch, d->character ) || d->original)
	    continue;
	wch   = d->original ? d->original : d->character;
	if ( wch->level[max_sec_level(wch)] < iLevelLower
	||   wch->level[max_sec_level(wch)] > iLevelUpper
	|| ( fImmortalOnly  && wch->level[max_sec_level(wch)] < LEVEL_IMMORTAL )
/*	|| ( fClassRestrict && !rgfClass[wch->class] ) Multiclassing Kyorlin */
	...
	sprintf( class_text, "%s%s %s", NOT_AUTHED(wch) ? "N" : " ", who_level(wch), who_classes(wch) );
	class = class_text;
	switch ( wch->level[max_sec_level(wch)] )
	{
	default: break;
	...
	case MAX_LEVEL - 15: class = "Grand Avatar";	break;
	case MAX_LEVEL - 16: class = "Avatar";		break;
        case MAX_LEVEL - 17: class = "Lord of Spirit";  break;
	case MAX_LEVEL - 18: class = "Master of Spirit"; break;
	case MAX_LEVEL - 19: class = "Initiate of Spirit"; break;
	case MAX_LEVEL - 20: class = "Lord of the Elements"; break;
	case MAX_LEVEL - 21: class = "Lord of Air"; 	break;
 	case MAX_LEVEL - 22: class = "Lord of Earth";   break;
	case MAX_LEVEL - 23: class = "Lord of Fire";	break;
 	case MAX_LEVEL - 24: class = "Lord of Water"; 	break;
	case MAX_LEVEL - 25: class = "Initiate Avatar"; break;
	}
	...
	sprintf( buf, "%*s%-15s %s%s%s%s%s%s%s%s.%s%s%s\n\r",
	  ...
	  IS_SET(wch->pcdata->flags, PCFLAG_QUEST) ? "(Questing) " : "",
...
void do_where( CHAR_DATA *ch, char *argument )
{
    ...
    {
  	send_to_char( "The gods forbid such information while questing", ch);
	return;
    }
...
void do_consider( CHAR_DATA *ch, char *argument )
{
    ...
    diff = victim->level[max_sec_level(victim)] - ch->level[max_sec_level(ch)];
...
/*          send_to_char_color( !IS_SET( ch->deaf, CHANNEL_PRAY )       ?
	...
	if (( fClear ) && ( ClearAll ))
	{
       ...
       */
	    ...
	    if (ch->level[max_sec_level(ch)] >= sysdata.log_level )
...
/*	else if ( !str_prefix( arg+1, "flee"     ) ) bit = PLR_FLEE; */
	...
	if (bit)
	  ...
	  if ( fSet )
	  {
	    sprintf( buf, "Ok, %s has been activated.", arg+1 );
	    send_to_char(buf,ch);
	    xSET_BIT   (ch->act, bit);
	  }
	  else
	  {
	    sprintf( buf, "Ok, %s has been deactivated.", arg+1 );
	    send_to_char( buf, ch);
...
void do_slist( CHAR_DATA *ch, char *argument )
{
   ...
   if (arg3[0]=='\0')
   {
	send_to_char("You must specify a class", ch);
	return;
   }
   if (arg1[0]!='\0')
      lowlev=atoi(arg1);
   ...
   for (i=lowlev; i <= hilev; i++)
   {
	...
	for ( sn = 0; sn < top_sn; sn++ )
	{
	    ...
	    if(i==skill_table[sn]->skill_level[skill_level(ch, sn)]  )
	    { 
		if( !lFound )
		{
		    lFound=1;
		...
		pager_printf(ch, "%7s: %20.20s \t Current: %-3d Max: %-3d  MinPos: %s \n\r",
                        ...
                        skill_table[sn]->skill_adept[skill_level(ch, sn)],
			buf );
	    }
	}
   }
