...
"stoponfail", "nofight", "nodispel", "randomtarget", "r2", "r3", "r4", "r5", "r6",
"r7", "r8", "r9", "r10", "r11"
};
...
bool check_skill( CHAR_DATA *ch, char *command, char *argument )
{
    ...
    if ( skill_table[sn]->type == SKILL_SPELL && skill_table[sn]->min_mana )
    {
	mana = IS_NPC(ch) ? 0 : UMAX(skill_table[sn]->min_mana,
	   100 / ( 2 + ch->level[skill_level(ch, sn)] - skill_table[sn]->skill_level[skill_level(ch, sn)] ) );
	...
	if ( !IS_NPC(ch) && ch->mana < mana && skill_table[sn]->type == SKILL_SPELL )
	{
...
*/
	...
	retcode = (*skill_table[sn]->spell_fun) ( sn, ch->level[skill_level(ch, sn)], ch, vo );
	end_timer(&time_used);
...
void do_skin( CHAR_DATA *ch, char *argument)
{
    ...
    korps               = create_object( get_obj_index(OBJ_VNUM_CORPSE), 0 );
    ...
    act( AT_MAGIC, "\nThe skinless corpse quickly falls apart without the skin which bound it...", ch, corpse, NULL, TO_CHAR);
    act( AT_MAGIC, "\nThe skinless corpse quickly falls apart without the skin which bound it together...", ch, corpse, NULL, TO_ROOM);
    extract_obj( corpse ); 
    obj_to_char( skin, ch );
    return;
}
...
void do_slookup( CHAR_DATA *ch, char *argument )
{
	...
	ch_printf( ch, "Sectors Allowed: %s\n",
 	...
 	ch_printf( ch, "Dammsg: %s\n\rWearoff: %s\n",
		skill->noun_damage, skill->msg_off );
...
/*	    char bitvector[MAX_INPUT_LENGTH];	*/
	...
	for ( sn = 0; sn < top_sn; sn++ )
	{
	    ...
	    if ( skill_table[sn]->name 
            && ( victim->level[skill_level(victim, sn)] >= skill_table[sn]->skill_level[skill_level(victim, sn)] 
                      || value == 0 ) )
	    {
...
void learn_from_success( CHAR_DATA *ch, int sn )
{
    ...
    sklvl = skill_table[sn]->skill_level[skill_level(ch, sn)];
    if( sklvl > ch->level[skill_level(ch, sn)] )
	return;
    if ( sklvl == 0 )
	...
	{
	    gain = 1000 * sklvl;
            if ( xIS_SET(ch->class,CLASS_MAGE) || xIS_SET( ch->class, CLASS_NECROMANCER) ) gain *= 5;	/* h, mage upgrade */
            if ( xIS_SET( ch->class, CLASS_CLERIC ) ) gain *= 3;	/* h, mage upgrade */
	...
	{
	    gain = 20 * sklvl;
            if ( xIS_SET( ch->class, CLASS_MAGE ) || xIS_SET( ch->class, CLASS_NECROMANCER) ) gain *= 6;	/* h, mage upgrade */
            if ( xIS_SET( ch->class, CLASS_CLERIC ) ) gain *= 3;	/* h, mage upgrade */
	 ...
	 ch->exp[min_sec_level(ch)] = UMAX( 0, ch->exp[min_sec_level(ch)] + gain );
         while ( ch->level[max_sec_level(ch)] < LEVEL_AVATAR && 
                 ch->exp[min_sec_level(ch)] >= exp_level(ch,
                 ch->level[max_sec_level(ch)]+1, max_sec_level(ch)))
        {
        	set_char_color( AT_WHITE + AT_BLINK, ch );
         	ch_printf( ch, "You have now obtained experience level %d!\n\r", ++ch->level[max_sec_level(ch)] );
        	advance_level( ch, min_sec_level(ch) );
	}
    }
}
...
{
    int adept, chance;
    return; //Disable learn from failure and free bonuses for adepting thru failure
    if ( IS_NPC(ch) || ch->pcdata->learned[sn] <= 0 )
	return;
...
void do_gouge( CHAR_DATA *ch, char *argument )
{
    ...
    {
	dam = number_range( 5, ch->level[max_sec_level(ch)] );
	...
	if ( global_retcode == rNONE )
	{
		if ( !IS_AFFECTED( victim, AFF_BLIND ) )
		{
		  ...
		  if ( !IS_NPC( victim ) && !IS_NPC( ch ) )
		    af.duration = ( ch->level[max_sec_level(ch)]+10 ) / get_curr_con( victim );
		  else
		    af.duration  = 3 + (ch->level[max_sec_level(ch)]/15);
...
/*	    WAIT_STATE( ch, skill_table[gsn_detrap]->beats ); */
    ...
    percent  = number_percent( ) - ( ch->level[max_sec_level(ch)] / 15 ) 
               - (get_curr_lck(ch) - 16);
	...
	default:
	  ...
	  else
	  {
		  ...
		  case SECT_UNDERWATER:
		    send_to_char( "The hole fills up as fast as you dig it",ch );
		    return;
	      }
	  }
...
void do_search( CHAR_DATA *ch, char *argument )
{
    ...
    percent  = number_percent( ) + number_percent( ) - ( ch->level[max_sec_level(ch)] / 10 );
...
void do_steal( CHAR_DATA *ch, char *argument )
{
    ...
    if ( get_timer(ch, TIMER_PKILLED) > 0 )
    {
        ...
        send_to_char( "You have just forfieted your pk grace period.\n\r", ch);
    ...
    if ( victim->fighting )    
    {
        send_to_char( "You can't get close enough to them at the moment", ch );
        return;
    }
    if ( victim == ch )
    {
...
*/
    ...
    */
    if( ch->level[max_sec_level(ch)] + 10 < victim->level[min_sec_level(victim)] )
    {
    ...
    ||   obj->level > ch->level[max_sec_level(ch)] )
    {
...
void do_dblbackstab( CHAR_DATA *ch, char *argument )
{
char arg[MAX_INPUT_LENGTH];
    CHAR_DATA *victim;
    OBJ_DATA *obj;
    ...
    WAIT_STATE( ch, skill_table[gsn_dblbackstab]->beats );
    if ( !IS_AWAKE(victim)
      || can_use_skill( ch, percent, gsn_dblbackstab ) )
    {
	learn_from_success( ch, gsn_dblbackstab );
	global_retcode = multi_hit( ch, victim, gsn_dblbackstab );
        global_retcode = multi_hit( ch, victim, gsn_dblbackstab );
    ...
    {
	learn_from_failure( ch, gsn_dblbackstab );
	global_retcode = damage( ch, victim, 0, gsn_dblbackstab );
        check_illegal_pk( ch, victim );
    }
    return;
}
...
void do_kick( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch)
    &&   ch->level[skill_level(ch, gsn_kick)] < skill_table[gsn_kick]->skill_level[skill_level( ch, gsn_kick )] )
    {
	send_to_char(
	...
	global_retcode = damage( ch, victim, number_range( 1, ch->level[skill_level(ch, gsn_kick)] ), gsn_kick );
    }
    else
    {
...
void do_punch( CHAR_DATA *ch, char *argument )
{
	...
	global_retcode = damage( ch, victim, number_range( 1, ch->level[skill_level(ch, gsn_punch)] ), gsn_punch );
    }
    else
    {
...
void do_bite( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch)
    &&   ch->level[skill_level(ch, gsn_bite)] < skill_table[gsn_bite]->skill_level[skill_level(ch, gsn_bite)] )
    {
	send_to_char(
	...
	global_retcode = damage( ch, victim, number_range( 1, ch->level[skill_level(ch, gsn_bite)] ), gsn_bite );
    }
    else
    {
...
void do_claw( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch)
    &&   ch->level[skill_level(ch, gsn_claw)] < skill_table[gsn_claw]->skill_level[skill_level(ch, gsn_claw)] )
    {
	send_to_char(
	    "That isn't quite one of your natural skills.\n\r", ch );
	return;
	...
	global_retcode = damage( ch, victim, number_range( 1, ch->level[skill_level(ch, gsn_claw)] ), gsn_claw );
    }
    else
    {
...
void do_sting( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch)
    &&   ch->level[skill_level(ch, gsn_sting)] < skill_table[gsn_sting]->skill_level[skill_level(ch, gsn_sting)] )
    {
	send_to_char(
	    "That isn't quite one of your natural skills.\n\r", ch );
	return;
	...
	global_retcode = damage( ch, victim, number_range( 1, ch->level[skill_level(ch, gsn_sting)] ), gsn_sting );
    }
    else
    {
...
void do_tail( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch)
    &&   ch->level[skill_level(ch, gsn_tail)] < skill_table[gsn_tail]->skill_level[skill_level(ch, gsn_tail) ] )
    {
	send_to_char(
	    "That isn't quite one of your natural skills.\n\r", ch );
	return;
  ...
  if ( can_use_skill( ch, number_percent(), gsn_tail ) )
	...
	global_retcode = damage( ch, victim, number_range( 1, ch->level[best_fighting_class(ch)] ), gsn_tail );
    }
    else
    {
...
void do_bash( CHAR_DATA *ch, char *argument )
{
    ...
    /*   
    if ( !IS_NPC(ch)
         &&   !CanUseSkill(ch, gsn_bash) )
    {
        send_to_char(
                     "You better leave the martial arts to fighters.\n\r", ch );
        return;
	}custom not work message (for fallen paladins and antipaladins too */
    ...
    /*  if ((IS_NPC(victim) && IS_SET(victim->act, ACT_HUGE)) ||
        ...
        if (!IsGiant(ch) && !IS_SET(ch->affected_by2, AFF2_GROWTH))
        {
            ...
            return;
	    } add enlarge for druid/mage*/
    
    chance = ( (get_curr_dex(victim) + get_curr_str(victim) + victim->level[best_fighting_class(victim)])
               -   (get_curr_dex(ch) + get_curr_str(ch) + ch->level[best_fighting_class(ch)]) );
    if ( victim->fighting && victim->fighting->who != ch )
        chance += 25;
...
void do_stun( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch)
    &&   ch->level[skill_level(ch, gsn_stun)] < skill_table[gsn_stun]->skill_level[skill_level(ch, gsn_stun)] )
    {
	send_to_char(
	    "You better leave the martial arts to fighters.\n\r", ch );
	return;
    ...
    fail = FALSE;
    chance = ris_save( victim, ch->level[max_sec_level(ch)], RIS_PARALYSIS );
    if ( chance == 1000 )
      fail = TRUE;
    else
        ...
        if ( can_use_skill( ch, number_percent(), gsn_feed ) )
	{
	  ...
	  &&  ch->pcdata->condition[COND_BLOODTHIRST] < (10 + ch->level[4]) )
	  {
	    gain_condition( ch, COND_BLOODTHIRST,
	     UMIN( number_range(1, (ch->level[4]+victim->level[max_sec_level(victim)] / 20) + 3 ),
	     (10 + ch->level[4]) - ch->pcdata->condition[COND_BLOODTHIRST] ) );
...
void disarm( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	obj->value[5] = victim->level[max_sec_level(ch)];
    }
    if ( IS_NPC(victim)
    || ( IS_OBJ_STAT(obj, ITEM_LOYAL) && !IS_NPC(ch) ) )
...
void do_disarm( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch)
    &&   ch->level[skill_level(ch, gsn_disarm)] < skill_table[gsn_disarm]->skill_level[skill_level(ch, gsn_disarm)] )
    {
    ...
    percent = number_percent( ) + victim->level[best_fighting_class(victim)] - ch->level[best_fighting_class(ch)]
...
void do_mistwalk( CHAR_DATA *ch, char *argument )
{
    ...
    ||  (IS_NPC(victim) && saves_spell_staff( ch->level[best_magic_class(ch)], victim ))
...
void do_pick( CHAR_DATA *ch, char *argument )
{
    ...
    {
	if ( IS_NPC(gch) && IS_AWAKE(gch) && ch->level[max_sec_level(ch)] + 5 < gch->level[min_sec_level(gch)] )
	{
...
void do_sneak( CHAR_DATA *ch, char *argument )
{
    ...
    {
	af.type      = gsn_sneak;
	af.duration  = ch->level[max_sec_level(ch)] * DUR_CONV;
...
void do_recall( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC( ch ) && !location && ch->level[max_sec_level(ch)] >= 5
	...
	{
	    WAIT_STATE( ch, 4 );
	    lose = ch->level[max_sec_level(ch)];
	    if ( ch->desc )
	      lose /= 2;
	    gain_exp( ch, 0 - lose );
	...
	lose = (exp_level(ch, ch->level[max_sec_level(ch)]+1, max_sec_level(ch)) - exp_level(ch, ch->level[max_sec_level(ch)], max_sec_level(ch))) * 0.02;
	if ( ch->desc )
	  lose /= 2;
	gain_exp( ch, 0 - lose );
...
void do_mount( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch)
    &&   ch->level[skill_level(ch, gsn_mount)] < skill_table[gsn_mount]->skill_level[skill_level(ch, gsn_mount)] )
    {
	send_to_char(
...
bool check_parry( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    {
	/* Tuan was here.  :) */
	chances	= UMIN( 60, 2 * victim->level[best_fighting_class(victim)] );
    }
    else
    {
    ...
    if ( !chance( victim, chances + victim->level[best_fighting_class(victim)] - ch->level[best_fighting_class(ch)] ) )
    {
...
bool check_tumble( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( xIS_SET( victim->class, CLASS_THIEF ) || !IS_AWAKE( victim ) )
      return FALSE;
    if ( !IS_NPC( victim )
...
void do_poison_weapon( CHAR_DATA *ch, char *argument )
{
  ...
  if ( !IS_NPC( ch )
  && ch->level[skill_level(ch, gsn_poison_weapon)] < skill_table[gsn_poison_weapon]->skill_level[skill_level(ch, gsn_poison_weapon)] )
    {
  ...
  one_argument( argument, arg );
    ...
    if ( !IS_NPC( ch ) && (get_curr_wis( ch ) + get_curr_int(ch) ) < 32 )
    {
	...
	damage( ch, ch, ch->level[skill_level(ch, gsn_poison_weapon )], gsn_poison_weapon );
    ...
    obj->timer = UMIN( obj->level, ch->level[skill_level(ch, gsn_poison_weapon)] );
...
void do_scribe( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch)
    &&   ch->level[skill_level(ch, gsn_scribe)] < skill_table[gsn_scribe]->skill_level[skill_level(ch, gsn_scribe)] )
    {
	...
	return;
     ...
     scroll->value[1] = sn;
     scroll->value[0] = ch->level[skill_level(ch, sn)];
...
void do_brew( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch)
    &&   ch->level[skill_level(ch, gsn_brew)] < skill_table[gsn_brew]->skill_level[skill_level(ch, gsn_brew)] )
    {
        send_to_char( "A skill such as this requires more magical ability than that of your class.\n\r", ch );
	return;
    }
	...
	return;
     ...
     potion->value[1] = sn;
     potion->value[0] = ch->level[skill_level(ch, sn)];
...
bool check_grip( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( IS_NPC(victim) )
	chance  = UMIN( 110, 2 * victim->level[best_fighting_class(victim)] );
    else
        chance  = LEARNED(victim, gsn_grip);

    /* Consider luck as a factor */
    chance += (2 * (get_curr_lck(victim) - 13 ) );

    if ( number_percent( ) >= chance + victim->level[best_fighting_class(victim)] - ch->level[best_fighting_class(ch)] )
    {
...
void do_berserk( CHAR_DATA *ch, char *argument )
{
  ...
  /* Hmmm.. 10-20 combat rounds at level 50.. good enough for most mobs,
     ...
     too high. -- Altrag */
  af.duration = number_range(ch->level[skill_level(ch, gsn_berserk)]/5, ch->level[skill_level(ch, gsn_berserk)]*2/5);
...
void do_hitall( CHAR_DATA *ch, char *argument )
{
  ...
  for ( vch = ch->in_room->first_person; vch; vch = vch_next )
  {
    ...
    if ( is_same_group(ch, vch) || !is_legal_kill(ch, vch) ||
      ...
      continue;
    if ( ++nvict > ch->level[skill_level(ch, gsn_hitall)] / 5 )
      break;
...
void do_elemental_fury( CHAR_DATA *ch, char *argument )
{
  sh_int percent;
  AFFECT_DATA af;
  CHAR_DATA *vch;
  ...
  if (ch->fighting )
  {
    send_to_char( "You are not able to break free of the fight!\n\r", ch );
    return;
  }
  ...
  percent = LEARNED(ch, gsn_elemental_fury);
  WAIT_STATE(ch, skill_table[gsn_elemental_fury]->beats);
  if ( !chance(ch, percent) )
  {
    send_to_char( "You couldn't build up enough rage.\n\r", ch);
    learn_from_failure(ch, gsn_elemental_fury);
    return;
  }
   for ( vch = ch->in_room->first_person; vch; vch = vch_next )
  {
    ...
    if ( is_same_group(ch, vch) || !is_legal_kill(ch, vch) ||
        !can_see(ch, vch) || is_safe(ch, vch, TRUE) )
      continue;
    if ( ++nvict > ch->level[skill_level(ch, gsn_elemental_fury)] / 5 )
      break;
    check_illegal_pk(ch, vch);
    if ( chance(ch, percent) )
    {
  ...
  af.type = gsn_elemental_fury;
  af.duration = number_range(ch->level[skill_level(ch, gsn_elemental_fury)]/5, ch->level[skill_level(ch, gsn_elemental_fury)]*2/5);
  af.location = APPLY_STR + APPLY_HITROLL + APPLY_DAMROLL;
  af.modifier = 5;
  af.bitvector = meb(AFF_ELEMENTAL_FURY);
  affect_to_char(ch, &af);
  send_to_char( "You let loose a furious warcry and enter combat!\n\r", ch );
  learn_from_success(ch, gsn_elemental_fury);
}    
...
bool check_illegal_psteal( CHAR_DATA *ch, CHAR_DATA *victim )
{
    if (!IS_NPC (victim) && !IS_NPC(ch))
    {
	if( ch->level[max_sec_level(ch)] - victim->level[max_sec_level(victim)] > 10
...
void do_scan( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->level[max_sec_level(ch)] < 50 ) --max_dist;
    if ( ch->level[max_sec_level(ch)] < 40 ) --max_dist;
    if ( ch->level[max_sec_level(ch)] < 30 ) --max_dist;
	...
	&&   ch->level[max_sec_level(ch)] < LEVEL_GREATER )
	{
	...
	OBJ_DATA *projectile, sh_int dist, sh_int dt, char *stxt, sh_int color )
{
    ...
    if ( IS_NPC(victim) && xIS_SET(victim->act, ACT_GUARD)
	...
	if ( projectile )
	{
	     ...
	     {
		 case 13:
               learn_from_failure( ch, gsn_crossbows );
		 break;
    ...
    */
    switch ( projectile->value[3] )
    {
	case 13:    wtype = gsn_crossbows;      break;
	...
	if ( projectile )
	    ...
	    global_retcode = spell_attack( dt, ch->level[best_magic_class(ch)], ch, victim );
    }
    else
    {
	switch ( projectile->value[3] )
...
void do_slice( CHAR_DATA *ch, char *argument )
{
  ...
  &&   ch->level[skill_level(ch, gsn_slice)] < skill_table[gsn_slice]->skill_level[skill_level(ch, gsn_slice)] )
  {
  ...
  if (corpse->item_type != ITEM_CORPSE || corpse->timer < 5 || corpse->value[3] < 75)
  {
  ...
  { 
    ...
    if ( number_percent() + (get_curr_dex(ch) - 13) < 10)   
    {
      ...
      damage(ch, ch, ch->level[skill_level(ch, gsn_slice)], gsn_slice);
    }
    return;
  }
...
/*  char buf[MAX_INPUT_LENGTH];
	...
	ch_printf_color( ch, "&wAdopt which fighting style?  (current:  %s&w)\n\r",
	  ...
	  ch->style == STYLE_TIGER      ? "&Bcircle of tigers" :
                                          "standard"     );
	return;
    }

    if( !str_prefix(arg, "evasive") ){
      if( ch->level[skill_level(ch, gsn_style_evasive)] < skill_table[gsn_style_evasive]->skill_level[skill_level(ch, gsn_style_evasive)])
      {
      ...
      if( ch->level[skill_level(ch, gsn_style_defensive)] < skill_table[gsn_style_defensive]->skill_level[skill_level(ch, gsn_style_defensive)])
      {
      ...
      if( ch->level[skill_level(ch, gsn_style_standard)] < skill_table[gsn_style_standard]->skill_level[skill_level(ch, gsn_style_standard)])
      {
      ...
      if( ch->level[skill_level(ch, gsn_style_aggressive)] < skill_table[gsn_style_aggressive]->skill_level[skill_level(ch, gsn_style_aggressive)])
      {
    ...
    } else if( !str_prefix(arg,"circle")){
      if( ch->level[skill_level(ch, gsn_tiger)] < skill_table[gsn_tiger]->skill_level[skill_level(ch, gsn_tiger)])
      {
	 send_to_char( "You have not yet learned enough to master this form.\n\r",ch );
	 return;
      }
      WAIT_STATE( ch, skill_table[gsn_tiger]->beats );
      ch->style = STYLE_TIGER;   /* STYLES BY KYORLIN */
      send_to_char("You raise your palms high in the air and prepare to embrace the style of the circling tiger!", ch);
      return;
    } else if( !str_prefix(arg,"berserk")){
      if( ch->level[skill_level(ch, gsn_style_berserk )] < skill_table[gsn_style_berserk]->skill_level[skill_level(ch, gsn_style_berserk)])
      {
...
void do_cook ( CHAR_DATA *ch, char *argument )
{
   ...
   one_argument( argument, arg );
   if ( IS_NPC(ch) || ch->level[skill_level(ch, gsn_cook)] < skill_table[gsn_cook]->skill_level[skill_level(ch, gsn_cook)])
   {
...
void do_sharpen( CHAR_DATA *ch, char *argument )
{
  ...
  if (! IS_NPC( ch )
  && ch->level[skill_level(ch, gsn_sharpen)] < skill_table [gsn_sharpen]->skill_level [skill_level(ch, gsn_sharpen)] )
    {
 ...
 one_argument ( argument, arg );
   ...
   && percent > ch->pcdata->learned[gsn_sharpen] )
   {
       ...
       set_char_color( AT_GREY, ch );
       damage( ch, ch, ch->level[skill_level(ch, gsn_sharpen)], gsn_sharpen );
