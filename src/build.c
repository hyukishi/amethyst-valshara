...
"dig", "eatkey", "nopassdoor", "hidden", "passage", "portal", "noscry", "r2",
...
"nosupplicate", "arena", "nomissile", "r4", "noexit", "prototype", "r6"
};
...
"donation", "clanobject", "corpse", "antivampire", "antidruid", 
"hidden", "poisoned", "covering", "deathrot", "buried", "prototype", 
"nolocate", "groundrot", "antinecromancer", "antipaladin", "antiaugurer", "antiranger" };
...
"corpse", "corpse2", "fountain", "pill", "blood", "bloodstain",
...
"nstart", "dnd", "r11", "r12", "quest", "r14", "r15", "r16", "r17", "r18",
...
void do_goto( CHAR_DATA *ch, char *argument )
{
	...
	||   vnum < 1 || IS_NPC(ch) || (!ch->pcdata->area && get_trust( ch ) < LEVEL_ASCENDANT) )
	{
...
void do_mset( CHAR_DATA *ch, char *argument )
{
        ...
        if( IS_NPC(victim) )
	{
	  ...
	  xTOGGLE_BIT( victim->class, value );
	  if(victim->level[value] < 1)
	     victim->level[value] = 1;
	  if ( xIS_SET(victim->act, ACT_PROTOTYPE) )
	     xTOGGLE_BIT( victim->pIndexData->class, value);
	  return;
	}
	...
	victim->level[has_class(victim->pIndexData)] = value;
	if ( IS_NPC(victim) && xIS_SET(victim->act, ACT_PROTOTYPE) )
	  victim->pIndexData->level[has_class(victim->pIndexData)] = value;
	return;
    }
	...
	{
	    send_to_char( "Hp range is 1 to 2,000,000 hit points.\n\r", ch );
	    return;
	}
...
void do_redit( CHAR_DATA *ch, char *argument )
{
	...
	case SUB_ROOM_DESC:
	  ...
	  return;
        ...
        case SUB_ROOM_DESC_NITE:  /* ACK 201 - Dracones */
	  location = ch->dest_buf;
	  if ( !location )
	  {
		bug( "redit: sub_room_desc: NULL ch->dest_buf", 0 );
	  ...
	  STRFREE( location->nitedesc );
	  location->nitedesc = copy_buffer( ch );
	  stop_editing( ch );
	  ch->substate = ch->tempnum;
	  return;
	...
	send_to_char( "\n\r",						ch );
	send_to_char( "Field being one of:\n\r",			ch );
	send_to_char( "  name desc nitedesc ed rmed light\n\r",		ch );
...
/*		    printf("%s\n\r", arg3);	*/
     ...
     if ( !str_cmp( arg, "light" ) )
    {
        if ( !argument || argument[0] == '\0' )
        {
           send_to_char( "Set the light level.\n\r",ch );
           send_to_char( "Usage: redit light <value>\n\r",ch);
        }
        location->light = atoi( argument );
...
void fold_area( AREA_DATA *tarea, char *filename, bool install )
{
	...
	||   pMobIndex->race	 != 0	||   xIS_SET( pMobIndex->class, CLASS_WARRIOR )
	...
	fprintf( fpout, "%d %d %d ",	pMobIndex->level[has_class(pMobIndex)],
					pMobIndex->mobthac0,
					pMobIndex->ac			);
...
/* Need to convert to new positions correctly on loadup sigh -Shaddai */
	...
	if ( complexmob )
	{
	  ...
	  fprintf( fpout, "%d %s %d %d %d %d %d\n",
					pMobIndex->race,
					print_bitvector(&pMobIndex->class),
					pMobIndex->height,
					pMobIndex->weight,
					pMobIndex->speaks,
					pMobIndex->speaking,
	...
	if ( val4 || val5 )
	  ...
	  fprintf( fpout, "%d %d %d %d\n",	val0, 
        ...
        /* ACK 202 check - Dracones */

	if( AREA_VERSION_WRITE == 202 || AREA_VERSION_WRITE > 210) {
	  fprintf( fpout, "%d %d %d %d\n",	pObjIndex->weight,
					pObjIndex->cost,
	...
	fprintf( fpout, "%s~\n",	strip_cr( room->description )	);
        ...
        /* ACK 201 check - Dracones */

	if( AREA_VERSION_WRITE == 201 || AREA_VERSION_WRITE > 210)
...
void do_mpedit( CHAR_DATA *ch, char *argument )
{
    ...
    if ( get_trust( ch ) < victim->level[max_sec_level(victim)] || !IS_NPC(victim) )
    {
	send_to_char( "You can't do that!\n\r", ch );
	return;
    }
...
void do_mpcopy( CHAR_DATA *ch, char *argument )
{
    ...
    if ( get_trust( ch ) < source->level[max_sec_level(source)] || !IS_NPC(source) || 
         get_trust( ch) < destination->level[max_sec_level(destination)] || !IS_NPC(destination) ) 
    {
	send_to_char( "You can't do that!\n\r", ch );
	return;
    }
...
void do_rdig( CHAR_DATA *ch, char *argument )
{
    ...
    location->nitedesc = STRALLOC(ch_location->nitedesc);
