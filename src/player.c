...
void do_worth(CHAR_DATA *ch, char *argument)
{
    ...
    pager_printf(ch, "|Level: %-4d |Favor: %-10s |Alignment: %-9s |Experience: %s|\n\r", 
                     ch->level[max_sec_level(ch)], buf, buf2, who_exp(ch) );
    send_to_pager(" ----------------------------------------------------------------------------\n\r", ch);
        switch (ch->style) {
        case STYLE_EVASIVE:
                sprintf(buf, "evasive");
    ...
    send_to_pager(" ----------------------------------------------------------------------------\n\r", ch);
    if ( ch->level[max_sec_level(ch)] < 15 && !IS_PKILL(ch) )
...
void do_score(CHAR_DATA * ch, char *argument)
{
    ...
    pager_printf(ch, "\n\rScore for %s%s.\n\r", ch->name, ch->pcdata->title);
    if ( get_trust( ch ) != ch->level[max_sec_level(ch)] )
	pager_printf( ch, "You are trusted at level %d.\n\r", get_trust( ch ) );
    
    send_to_pager("----------------------------------------------------------------------------\n\r", ch);

    pager_printf(ch, "LEVEL: %-3s         Race : %-10.10s        Played: %d hours\n\r",
	who_level(ch), capitalize(get_race(ch)), (get_age(ch) - 17) * 2);

    pager_printf(ch, "YEARS: %-6d      Class: %-11.11s       Log In: %s\r",
		get_age(ch), score_classes(ch), ctime(&(ch->logon)) );

    if (ch->level[max_sec_level(ch)] >= 15
    ||  IS_PKILL( ch ) )
    {
    ...
    if (ch->level[max_sec_level(ch)] > 24)
	...
	pager_printf(ch, "XP   : %-9d       Blood: %-5d of %5d   MKills:  %-5.5d    AutoLoot(%c)\n\r",
		ch->exp[max_sec_level(ch)], ch->pcdata->condition[COND_BLOODTHIRST], 10 + ch->level[max_sec_level(ch)], ch->pcdata->mkills,
		xIS_SET(ch->act, PLR_AUTOLOOT) ? 'X' : ' ');
    else if (xIS_SET(ch->class, CLASS_WARRIOR ) )
	pager_printf(ch, "XP   : %-9d                               MKills:  %-5.5d    AutoLoot(%c)\n\r",
		ch->exp[max_sec_level(ch)], ch->pcdata->mkills, xIS_SET(ch->act, PLR_AUTOLOOT) ? 'X' : ' ');
    else
	pager_printf(ch, "XP   : %-9d        Mana: %-5d of %5d   MKills:  %-5.5d    AutoLoot(%c)\n\r",
		ch->exp[max_sec_level(ch)], ch->mana, ch->max_mana, ch->pcdata->mkills, xIS_SET(ch->act, PLR_AUTOLOOT) ? 'X' : ' ');
	...
	for (paf = ch->first_affect; paf; paf = paf->next)
	{
		...
		continue;
	    if (ch->level[max_sec_level(ch)] < 20)
	    {
...
void do_newscore(CHAR_DATA * ch, char *argument)
{
    ...
    pager_printf_color(ch, "Level: &W%-3s         &CRace : &W%-10.10s        &CPlayed: &W%d &Chours\n\r",
	who_level(ch), capitalize(get_race(ch)), (get_age(ch) - 17) * 2);
    pager_printf_color(ch, "&CYears: &W%-6d      &CClass: &W%s       &CLog In: %s\r",
		get_age(ch), score_classes(ch), ctime(&(ch->logon)) );
    ...
    if (IS_VAMPIRE(ch))
	pager_printf_color(ch, "&CEXP  : &W%s       &CBlood: &R%-5d &Cof &r%5d   &CMKills:  &W%5d    &CAutoLoot(&W%c&C)\n\r",
		who_exp(ch), ch->pcdata->condition[COND_BLOODTHIRST], 10 + ch->level[max_sec_level(ch)], ch->pcdata->mkills,
		xIS_SET(ch->act, PLR_AUTOLOOT) ? 'X' : ' ');
    else if (xIS_SET(ch->class, CLASS_WARRIOR ) )
	pager_printf_color(ch, "&CEXP  : &W%s                               &CMKills:  &W%5d    &CAutoLoot(&W%c&C)\n\r",
		who_exp(ch), ch->pcdata->mkills, xIS_SET(ch->act, PLR_AUTOLOOT) ? 'X' : ' ');
    else
	pager_printf_color(ch, "&CEXP  : &W%s        &CMana: &B%-5d &Cof &b%5d   &CMKills:  &W%5d    &CAutoLoot(&W%c&C)\n\r",
		who_exp(ch), ch->mana, ch->max_mana, ch->pcdata->mkills, xIS_SET(ch->act, PLR_AUTOLOOT) ? 'X' : ' ');
...
{
    if ( max_sec_level(ch) < MAX_NPC_CLASS && max_sec_level(ch) >= 0)
	return ( npc_class[max_sec_level(ch)] );
    return ("Unknown");
}
...
void do_oldscore( CHAR_DATA *ch, char *argument )
{
	...
	ch->exp[max_sec_level(ch)],  ch->gold );
    ...
    if ( ch->level[max_sec_level(ch)] >= 25 )
...
void do_level( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->level[min_sec_level(ch)] == 1 )
      lowlvl = 1;
    else
      lowlvl = UMAX( 2, ch->level[min_sec_level(ch)] - 5 );
    hilvl = URANGE( ch->level[min_sec_level(ch)], ch->level[min_sec_level(ch)] + 5, MAX_LEVEL );
    set_char_color( AT_SCORE, ch );
    ch_printf( ch, "\n\rExperience required, levels %d to %d:\n\r______________________________________________\n\r\n\r", lowlvl, hilvl );
    sprintf( buf, " exp  (You have: %d)", ch->exp[min_sec_level(ch)] );
    sprintf( buf2," exp  (To level: %d)", exp_level(ch, ch->level[min_sec_level(ch)]+1,min_sec_level(ch)) - ch->exp[min_sec_level(ch)] );
    for ( x = lowlvl; x <= hilvl; x++ )
	ch_printf( ch, " (%2d) %11d%s\n\r", x, exp_level( ch, x, min_sec_level(ch) ),
		(x == ch->level[min_sec_level(ch)]) ? buf : (x == ch->level[min_sec_level(ch)]+1) ? buf2 : " exp" );
...
void do_remains( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->pcdata->favor < ch->level[max_sec_level(ch)]*2 ) {
    ...
    {
      send_to_pager( "\n\r", ch );
      ch->pcdata->favor -= ch->level[max_sec_level(ch)]*2;
    }
    return;
}
...
void do_title( CHAR_DATA *ch, char *argument )
{
    ...
    set_char_color( AT_SCORE, ch );
    if ( ch->level[max_sec_level(ch)] < 5 )
    {
...
void do_statreport( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_VAMPIRE(ch) )
    {
      ch_printf( ch, "You report: %d/%d hp %d/%d blood %d/%d mv %d xp.\n\r",
		 ...
		 10 + ch->level[max_sec_level(ch)], ch->move, ch->max_move, ch->exp[min_sec_level(ch)]   );
    ...
    else
    {
      ch_printf( ch, "You report: %d/%d hp %d/%d mana %d/%d mv %d xp.\n\r",
 		...
 		ch->move, ch->max_move, ch->exp[min_sec_level(ch)]   );
...
void do_report( CHAR_DATA *ch, char *argument )
{
	...
	ch->hit,  ch->max_hit,
	ch->pcdata->condition[COND_BLOODTHIRST], 10 + ch->level[max_sec_level(ch)],
	ch->move, ch->max_move,
	ch->exp[min_sec_level(ch)]   );
    else
      ch_printf( ch,
