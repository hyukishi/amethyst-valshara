...
{
    return ch->exp[min_sec_level(ch)];
}
...
int get_exp_worth( CHAR_DATA *ch )
{
    ...
    exp = ch->level[max_sec_level(ch)] * ch->level[max_sec_level(ch)] * ch->level[max_sec_level(ch)] * 5;
    exp += ch->max_hit;
    exp -= (ch->armor-50) * 2;
    exp += ( ch->barenumdie * ch->baresizedie + GET_DAMROLL(ch) ) * 10;
    exp += GET_HITROLL(ch) * ch->level[best_fighting_class(ch)] * 10;
...
sh_int get_exp_base( CHAR_DATA *ch, sh_int class )
{
    if ( IS_NPC(ch) )
      return 1000;
    return class_table[class]->exp_base;
}

/*								-Thoric
 * Return how much experience is required for ch to get to a certain level
 */
int exp_level( CHAR_DATA *ch, sh_int level, sh_int class )
{
   int lvl;

   lvl = UMAX(0, level - 1);
   return ( lvl * lvl * lvl * get_exp_base(ch, class) );
}
...
sh_int level_exp( CHAR_DATA *ch, int exp )
{
    ...
    while ( !y )
    {
	tmp = exp_level(ch, x, max_sec_level(ch) );
	lastx = x;
	if ( tmp > exp )
	  x /= 2;
	else
...
sh_int get_trust( CHAR_DATA *ch )
{
    ...
    if ( IS_NPC(ch) && ch->level[max_sec_level(ch)] >= LEVEL_AVATAR )
	return LEVEL_AVATAR;

    if ( ch->level[max_sec_level(ch)] >= LEVEL_NEOPHYTE && IS_RETIRED( ch ) )
      return LEVEL_NEOPHYTE;

    return ch->level[max_sec_level(ch)];
}
...
sh_int get_curr_str( CHAR_DATA *ch )
{
    ...
    if ( IS_NPC(ch) || class_table[best_fighting_class(ch)]->attr_prime == APPLY_STR )
	max = 25;
    else if (class_table[best_fighting_class(ch)]->attr_second == APPLY_STR )
        max = 22;
    else if (class_table[best_fighting_class(ch)]->attr_deficient == APPLY_STR)
        max = 16;
    else
	max = 20;
...
sh_int get_curr_int( CHAR_DATA *ch )
{
    ...
    if ( IS_NPC(ch) || class_table[best_magic_class(ch)]->attr_prime == APPLY_INT )
	max = 25;
    else if(class_table[best_magic_class(ch)]->attr_second == APPLY_INT )
        max = 22;
    else if(class_table[best_magic_class(ch)]->attr_deficient == APPLY_INT )
        max = 16;
    else
	max = 20;
...
sh_int get_curr_wis( CHAR_DATA *ch )
{
    ...
    if ( IS_NPC(ch) || class_table[best_magic_class(ch)]->attr_prime == APPLY_WIS )
	max = 25;
    else if(class_table[best_magic_class(ch)]->attr_second == APPLY_WIS )
        max = 22;
    else if(class_table[best_magic_class(ch)]->attr_deficient == APPLY_WIS )
        max = 16;
    else
	max = 20;
...
sh_int get_curr_dex( CHAR_DATA *ch )
{
    ...
    if ( IS_NPC(ch) || class_table[best_fighting_class(ch)]->attr_prime == APPLY_DEX )
	max = 25;
    else if(class_table[best_fighting_class(ch)]->attr_second == APPLY_DEX )
        max = 22;
    else if(class_table[best_fighting_class(ch)]->attr_deficient == APPLY_DEX )
        max = 16;
    else
	max = 20;
...
sh_int get_curr_con( CHAR_DATA *ch )
{
    ...
    if ( IS_NPC(ch) || class_table[best_fighting_class(ch)]->attr_prime == APPLY_CON )
	max = 25;
    else if(class_table[best_fighting_class(ch)]->attr_second == APPLY_CON )
        max = 22;
    else if(class_table[best_fighting_class(ch)]->attr_deficient == APPLY_CON )
        max = 16;
    else
	max = 20;
...
sh_int get_curr_cha( CHAR_DATA *ch )
{
    ...
    if ( IS_NPC(ch) || class_table[best_magic_class(ch)]->attr_prime == APPLY_CHA )
	max = 25;
    else if(class_table[best_magic_class(ch)]->attr_second == APPLY_CHA )
        max = 22;
    else if(class_table[best_magic_class(ch)]->attr_deficient == APPLY_CHA )
        max = 16;
    else
	max = 20;
...
sh_int get_curr_lck( CHAR_DATA *ch )
{
    ...
    if ( IS_NPC(ch) || class_table[best_fighting_class(ch)]->attr_prime == APPLY_LCK )
	max = 25;
    else if(class_table[best_fighting_class(ch)]->attr_second == APPLY_LCK )
        max = 22;
    else if(class_table[best_fighting_class(ch)]->attr_deficient == APPLY_LCK )
        max = 16;
    else
	max = 20;
...
int can_carry_n( CHAR_DATA *ch )
{
	...
	return ch->level[max_sec_level(ch)]*200;
    ...
    if ( get_eq_char(ch, WEAR_SHIELD) )
      ++penalty;
    return URANGE(20, (ch->level[max_sec_level(ch)]+15)/5 + get_curr_dex(ch)-13 - penalty, 40);
}
...
void affect_modify( CHAR_DATA *ch, AFFECT_DATA *paf, bool fAdd )
{
	...
	if ( !IS_NPC(ch) )
		...
		URANGE( 0, ch->pcdata->condition[COND_BLOODTHIRST] + mod, ch->level[4]+10 );
	break;
...
/* spell cast upon wear/removal of an object	-Thoric */
	...
	&&   skill->type == SKILL_SPELL )
	{
	   ...
	   if ( (retcode=(*skill->spell_fun) ( mod, ch->level[best_magic_class(ch)], ch, ch )) == rCHAR_DIED
	   ||   char_died(ch) )
	      return;
	}
	break;
...
void update_aris(CHAR_DATA *ch)
{
	...
	SET_BIT(ch->susceptible, race_table[ch->race]->suscept);
        ...
        /* Add in effects from class  fix this -- multiclassing kyorlin 
 	xSET_BITS(ch->affected_by, class_table[best_magic_class(ch)]->affected);
 	SET_BIT(ch->resistant, class_table[best_magic_class(ch)]->resist);
 	SET_BIT(ch->susceptible, class_table[best_magic_class(ch)]->suscept);
*/
...
void obj_from_room( OBJ_DATA *obj )
{
    ...
    obj->in_obj	      = NULL;
    obj->in_room      = NULL;
    if ( obj->pIndexData->vnum == OBJ_VNUM_CORPSE && falling < 1 )
...
bool room_is_dark( ROOM_INDEX_DATA *pRoomIndex )
{
    ...
    ||   pRoomIndex->sector_type == SECT_HIVE )
	return FALSE;
...
char *extra_bit_name( EXT_BV *extra_flags )
{
    ...
    if ( xIS_SET(*extra_flags, ITEM_ANTI_PALADIN)) strcat( buf, " anti-paladin" );
    if ( xIS_SET(*extra_flags, ITEM_ANTI_NECROMANCER)) strcat( buf, " anti-necromancer" );
    if ( xIS_SET(*extra_flags, ITEM_ANTI_AUGURER)) strcat( buf, " anti-augurer" );
    ...
    if ( xIS_SET(*extra_flags, ITEM_SHARP)	) strcat( buf, " sharpened"    );
    if ( xIS_SET(*extra_flags, ITEM_NOSAVE) 	) strcat( buf, " nosave"       );
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
...
void clean_mob( MOB_INDEX_DATA *mob )
{
	...
	mob->sex	 = 0;	   mob->level[has_class( mob )]	= 0;
...
bool in_soft_range( CHAR_DATA *ch, AREA_DATA *tarea )
{
  ...
  if ( ch->level[max_sec_level(ch)] >= tarea->low_soft_range || ch->level[min_sec_level(ch)] <= tarea->hi_soft_range )
    return TRUE;
  else
    return FALSE;
}
...
bool in_hard_range( CHAR_DATA *ch, AREA_DATA *tarea )
{
  ...
  if ( ch->level[max_sec_level(ch)] >= tarea->low_hard_range && ch->level[min_sec_level(ch)] <= tarea->hi_hard_range )
    return TRUE;
  else
    return FALSE;
}
...
void economize_mobgold( CHAR_DATA *mob )
{
    ...
    mob->gold = UMIN( mob->gold, mob->level[max_sec_level(mob)] * mob->level[min_sec_level(mob)] * 400 );
    if ( !mob->in_room )
	return;
