...
void do_mpstat( CHAR_DATA *ch, char *argument )
{
	...
	victim->level[max_sec_level(victim)],       max_sec_level(victim),        victim->alignment,
	GET_AC( victim ),    victim->gold,         victim->exp[max_sec_level(victim)] );
...
void do_mpadvance( CHAR_DATA *ch, char *argument )
{
	...
	level = victim->level[max_sec_level(victim)] + 1;

	if ( victim->level[max_sec_level(victim)] > ch->level[max_sec_level(ch)] )
	{
    ...
    for ( iLevel = victim->level[max_sec_level(victim)] ; iLevel < level; iLevel++ )
    {
	if (level < LEVEL_IMMORTAL)
	  send_to_char( "You raise a level!!  ", victim );
	victim->level[max_sec_level(victim)] += 1;
	advance_level( victim, max_sec_level(victim) );
    }
    victim->exp[max_sec_level(victim)]   = 1000 * UMAX( 1, victim->level[max_sec_level(victim)] );
    victim->trust = 0;
    return;
}
...
void do_mp_practice( CHAR_DATA *ch, char *argument )
{
    ...
    if(victim->level[skill_level(ch, sn)] < skill_table[sn]->skill_level[skill_level(ch, sn)] )
    {
...
ch_ret simple_damage( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
    ...
    */
	...
	if ( !npcvict )
	{
	     ...
	     */
	    if ( victim->exp[max_sec_level(victim)] > exp_level(victim, victim->level[max_sec_level(victim)], max_sec_level(victim)) )
		gain_exp( victim, (exp_level(victim, victim->level[max_sec_level(victim)], max_sec_level(victim) ) - victim->exp[max_sec_level(victim)])/2 );
	}
	set_cur_char(victim);
	raw_kill( ch, victim );
	victim = NULL;
