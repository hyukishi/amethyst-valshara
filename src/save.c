...
bool load_char_obj( DESCRIPTOR_DATA *d, char *name, bool preload )
{
    ...
    if ( stat( strsave, &fst ) != -1 )
    {
      if ( fst.st_size == 0 )
      {
	sprintf( strsave, "%s%c/%s", BACKUP_DIR, tolower(fname[0]),
			capitalize( fname ) );
...
void fread_char( CHAR_DATA *ch, FILE *fp, bool preload )
{
	...
	case 'I':
		...
		strcpy(filekey, player_filename(temp));
	    	
	    	sprintf(fname, "%s%c/%s", PLAYER_DIR,
	    		tolower(filekey[0]), capitalize(filekey));
...
void do_last( CHAR_DATA *ch, char *argument )
{
    ...
    strcpy( name, capitalize_name(arg) );
    strcpy( arg, player_filename(arg) );
