...
void boot_db( bool fCopyOver )
{
	...
	*/
	...
	ASSIGN_GSN( gsn_dblbackstab,	"doublebackstab" );
	...
	ASSIGN_GSN( gsn_ninth_attack,   "ninth attack" );
	ASSIGN_GSN( gsn_tenth_attack,	"tenth attack" );
	...
	ASSIGN_GSN( gsn_elemental_fury, "elemental fury");
...
void load_mobiles( AREA_DATA *tarea, FILE *fp )
{
	...
	if ( letter == 'C' ) /* Realms complex mob 	-Thoric */
	{
	    ...
	    pMobIndex->class		= fread_bitvector( fp );
	    for( i = 0; i <= MAX_CLASS; i++ )
	        if(xIS_SET(pMobIndex->class, i ) )
                   pMobIndex->level[i] = iLevel;
		pMobIndex->level[0] = iLevel; /* just to be safe */
	    ...
	    pMobIndex->numattacks	= fread_number( fp );
	    if ( !pMobIndex->speaks && ( pMobIndex->race < MAX_RACE) )
		pMobIndex->speaks = race_table[pMobIndex->race]->language | LANG_COMMON;
	    if ( !pMobIndex->speaking && ( pMobIndex->race < MAX_RACE) )
...
void load_objects( AREA_DATA *tarea, FILE *fp )
{
	...
	pObjIndex->value[5]		= x6;
	  /* ACK 200 compatibility - Dracones */
	...
	pObjIndex->level                = x4;
        ...
        if ( area_version == 1 || area_version >= 200 )
	{
...
void load_rooms( AREA_DATA *tarea, FILE *fp )
{
	...
	if( area_version > 200)
	  pRoomIndex->nitedesc = fread_string( fp );
	else pRoomIndex->nitedesc = STRALLOC( "" );
...
CHAR_DATA *create_mobile( MOB_INDEX_DATA *pMobIndex )
{
    ...
    mob->mpscriptpos		= 0;
    mob->level[has_class(pMobIndex)] = number_fuzzy( pMobIndex->level[has_class(pMobIndex)] );
    mob->act			= pMobIndex->act;

    if ( xIS_SET( mob->act, ACT_MOBINVIS) )
	mob->mobinvis = mob->level[max_sec_level(mob)];
	...
	mob->max_hit		= mob->level[has_class(pMobIndex)] * 8 + number_range(
					mob->level[has_class(pMobIndex)] * mob->level[has_class(pMobIndex)] / 4,
					mob->level[has_class(pMobIndex)] * mob->level[has_class(pMobIndex)] );
    else
    ...
    mob->exp[max_sec_level(mob)]	= pMobIndex->exp;
...
void clear_char( CHAR_DATA *ch )
{
    ...
    ch->xflags			= 0;
    ch->race			= 0;
    xTOGGLE_BIT( ch->class, CLASS_WARRIOR);
...
/*
 * Returns a player name with each word initial-capped.
 */
char *capitalize_name( const char *str )
{
	...
	if ( isalpha(str[i]) )
	{
	    strcap[i] = cap_next ? UPPER(str[i]) : LOWER(str[i]);
	    cap_next = FALSE;
	}
	else
	{
...
/*
 * Returns the pfile key for a player name.
 */
char *player_filename( const char *str )
{
...
ROOM_INDEX_DATA *make_room( int vnum )
{
	...
	pRoomIndex->description		= STRALLOC("");
        /* added NiteDesc -- Dracones */
        pRoomIndex->nitedesc		= STRALLOC("");
...
MOB_INDEX_DATA *make_mobile( sh_int vnum, sh_int cvnum, char *name )
{
	...
	if ( !cMobIndex )
	{
	  ...
	  pMobIndex->perm_lck		= 13;
	  pMobIndex->race		= 0;
	  xTOGGLE_BIT( pMobIndex->class, CLASS_WARRIOR );
	  pMobIndex->xflags		= 0;
	  pMobIndex->resistant		= 0;
	  pMobIndex->immune		= 0;
	  pMobIndex->susceptible	= 0;
	...
	else
	{
	  ...
	  pMobIndex->level[class]	= cMobIndex->level[class];
...
bool recent_create( sh_int size ) {
   if( size < 1 ) {
	 ...
	 CREATE( recent_first, RECENT_DATA, 1);
...
bool recent_add( CHAR_DATA *ch ) {
   ...
   if( recent_free == NULL || recent_first == NULL ) {
      if( !recent_create( 100 ) ) {
         bug("recent_add: call \'recent_create( 100 )\' failed.", 0 );
         return FALSE;
      }
   }
