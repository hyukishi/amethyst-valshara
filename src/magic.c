...
int ch_slookup( CHAR_DATA *ch, const char *name )
{
	...
	&&   ch->level[skill_level(ch, sn)] >= skill_table[sn]->skill_level[skill_level(ch, sn)] 
...
dispel_casting (AFFECT_DATA *paf,CHAR_DATA *ch,CHAR_DATA *victim,int affect, bool dispel)
{
	...
	EXT_BV ext_bv = meb(affect);
        ...
        if ( IS_NPC ( ch ) || xIS_SET (ch->class, CLASS_MAGE ) )
                is_mage = TRUE;
...
void say_spell( CHAR_DATA *ch, int sn )
{
    ...
    {
	if ( rch != ch )
	act( AT_MAGIC, best_magic_class(ch) == best_magic_class(rch) ? buf : buf2,
	     ch, NULL, rch, TO_VICT );
    }
...
bool saves_poison_death( int level, CHAR_DATA *victim )
{
    ...
    save = 50 + ( victim->level[best_fighting_class(victim)] - level - victim->saving_poison_death ) * 5;
...
bool saves_wands( int level, CHAR_DATA *victim )
{
    ...
    save = 50 + ( victim->level[best_magic_class(victim)] - level - victim->saving_wand ) * 5;
    save = URANGE( 5, save, 95 );
    return chance( victim, save );
}
bool saves_para_petri( int level, CHAR_DATA *victim )
{
    ...
    save = 50 + ( victim->level[best_magic_class(victim)] - level - victim->saving_para_petri ) * 5;
    save = URANGE( 5, save, 95 );
    return chance( victim, save );
}
bool saves_breath( int level, CHAR_DATA *victim )
{
    ...
    save = 50 + ( victim->level[best_magic_class(victim)] - level - victim->saving_breath ) * 5;
    save = URANGE( 5, save, 95 );
    return chance( victim, save );
}
bool saves_spell_staff( int level, CHAR_DATA *victim )
{
    ...
    if ( IS_NPC( victim ) && level > 10 )
      level -= 5;
    save = 50 + ( victim->level[best_magic_class(victim)] - level - victim->saving_spell_staff ) * 5;
    save = URANGE( 5, save, 95 );
    return chance( victim, save );
}
...
void do_cast( CHAR_DATA *ch, char *argument )
{
	...
	if ( get_trust(ch) < LEVEL_GOD )
	{
	    ...
	    || ( !IS_NPC(ch) && ch->level[skill_level(ch, sn)] < skill_table[sn]->skill_level[skill_level(ch, sn)] ) )
	    {
	...
	if ( !IS_NPC(ch) && ch->mana < mana && skill->type == SKILL_SPELL )
	{
	...
	if ( IS_VALID_SN((sn = ch->tempnum)) )
	{
	    ...
	    if (ch->level[max_sec_level(ch)] < LEVEL_IMMORTAL)    /* so imms dont lose mana */
	      ch->mana -= mana / 3;
	}
	set_char_color( AT_MAGIC, ch );
	...
	else
	{
	   ...
	   retcode = (*skill->spell_fun) ( sn, ch->level[skill_level(ch, sn)], ch, vo );
	   end_timer(&time_used);
...
ch_ret obj_cast_spell( int sn, int level, CHAR_DATA *ch, CHAR_DATA *victim, OBJ_DATA *obj )
{
    ...
    int levdiff = ch->level[max_sec_level(ch)] - level;
    SKILLTYPE *skill = get_skilltype( sn );
    struct timeval time_used;
...
ch_ret spell_charm_person( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    ||   level < victim->level[max_sec_level(victim)]
...
/*    act( AT_MAGIC, "Isn't $n just so nice?", ch, NULL, victim, TO_VICT );
    ...
    log_string_plus( buf, LOG_NORMAL, ch->level[max_sec_level(ch)] );
    if ( IS_NPC( victim ) )
    {
...
/*
    to_channel( buf, CHANNEL_MONITOR, "Monitor", UMAX( LEVEL_IMMORTAL, ch->level[max_sec_level(ch)] ) );
*/
    return rNONE; 
}
...
ch_ret spell_control_weather( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    change = number_range(-rand_factor, rand_factor) +
    	(ch->level[best_magic_class(ch)]*3)/(2*max_vector);
  ...
  if ( IS_NPC( victim ) && !xIS_EMPTY(victim->affected_by) )
  {
        ...
        if ( found ) /* Ok lets see if it is a spell */
        {
          ...
          if ( paf ) /*It is a spell lets remove the spell too */
          {
                ...
                if (level < victim->level[best_magic_class(victim)] || saves_spell_staff (level, victim))
                {
          ...
          {
                if (level < victim->level[max_sec_level(victim)] || saves_spell_staff (level, victim))
                {
...
ch_ret spell_energy_drain( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    chance = ris_save( victim, victim->level[max_sec_level(victim)], RIS_DRAIN );
    ...
    if ( victim->level[max_sec_level(victim)] <= 2 )
	dam		 = ch->hit + 1;
    else
    {
...
/*		obj->name,*/
	...
	name,
	victim->level[min_sec_level(victim)] - (victim->level[min_sec_level(victim)] % 5), 
        victim->level[max_sec_level(victim)] - (victim->level[max_sec_level(victim)] % 5) + 5);
 
    if ( IS_NPC(victim) && victim->morph )
	ch_printf(ch,"%s appears to truly be %s.\n\r",
	name, 
	(ch->level[max_sec_level(ch)] > victim->level[min_sec_level(victim)] + 10) 
    ...
    if ( (chance(ch, 50) && ch->level[max_sec_level(ch)] >= victim->level[min_sec_level(victim)] + 10 )
    ||    IS_IMMORTAL(ch) )
    {
...
ch_ret spell_sleep( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    ||   level < victim->level[max_sec_level(ch)]
	...
	log_string_plus( log_buf, LOG_NORMAL, ch->level[max_sec_level(ch)] );
	to_channel( log_buf, CHANNEL_MONITOR, "Monitor", UMAX( LEVEL_IMMORTAL, ch->level[max_sec_level(ch)] ) );
    }
...
ch_ret spell_summon( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    char buf[MAX_STRING_LENGTH];
    SKILLTYPE *skill = get_skilltype(sn);
    if( !IS_NPC(ch) && IS_SET(ch->pcdata->flags, PCFLAG_QUEST) ) 
    {
        send_to_char( "Use of this spell during a quest is prohibited", ch);
        return rSPELL_FAILED;      
    }
    ...
    ||   (IS_SET(victim->in_room->room_flags, ROOM_SAFE) && IS_SET(victim->pcdata->flags, PCFLAG_NOSUMMON ) )
    ||   IS_SET(ch->in_room->room_flags, ROOM_PRIVATE)
    ||   IS_SET(ch->in_room->room_flags, ROOM_SOLITARY)
    ||   IS_SET(victim->in_room->room_flags, ROOM_NO_SUMMON)
    ||   ( victim->level[min_sec_level(victim)] >= level + 3 && IS_NPC(victim) )
    ||   victim->fighting
    ...
    if( !IS_NPC(victim) && IS_SET(victim->pcdata->flags, PCFLAG_QUEST ) )
    {
        send_to_char( "You may not use this spell upon someone in a quest", ch );
        return rSPELL_FAILED;
    }
...
ch_ret spell_teleport( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    for ( ;; )
    {
	pRoomIndex = get_room_index( number_range( 0, 183647 ) );
	if ( pRoomIndex )
...
ch_ret spell_weaken( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( ch != victim )
    {
      if ( ( ( ( !IS_NPC(victim) && class_table[best_fighting_class(victim)]->attr_prime == APPLY_STR )
      || IS_NPC(victim) )
...
ch_ret spell_word_of_recall( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    send_to_char( "You utter the words, 'xafe ay candusqarr'.", ch);
...
ch_ret spell_transport( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    ||   victim->level[min_sec_level(victim)] >= level + 15
    ||	(IS_NPC(victim) && xIS_SET(victim->act, ACT_PROTOTYPE))
    ||  (IS_NPC(victim) && saves_spell_staff( level, victim )) )
    {
	failed_casting( skill, ch, victim, NULL );
...
ch_ret spell_recharge( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if ( chance(ch, 50 - (ch->level[best_magic_class(ch)]/2) ) )
	{
...
ch_ret spell_remove_invis( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if (victim)
	{
	    ...
	    if ( !IS_NPC(victim) )
	    {
		if ( chance(ch, 50) && ch->level[skill_level(ch, sn)] + 10 < victim->level[max_sec_level(victim)] )
		{
		    failed_casting( skill, ch, victim, NULL );
		    return rSPELL_FAILED;
		}
	    ...
	    {
		if ( chance(ch, 50) && ch->level[skill_level(ch, sn)] + 15 < victim->level[max_sec_level(victim)] )
		{
		    failed_casting( skill, ch, victim, NULL );
		    return rSPELL_FAILED;
		}
...
ch_ret spell_animate_dead( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if (corpse->item_type == ITEM_CORPSE && corpse->cost != -5)
	{
	   found = TRUE;
	   break;
	}
    ...
    if ( !IS_NPC(ch) )
    {
      if ( IS_VAMPIRE(ch) )
      {
        if ( !IS_IMMORTAL(ch) && ch->pcdata->condition[COND_BLOODTHIRST] -
            (pMobIndex->level[3]/3) < 0 )
        {
          send_to_char("You do not have enough blood power to reanimate this"
                      " corpse.\n\r", ch );
          return rSPELL_FAILED;
        }
        gain_condition(ch, COND_BLOODTHIRST, pMobIndex->level[3]/3);
      }
      else if ( ch->mana - (pMobIndex->level[3]*4) < 0 )
      {
	...
	return rSPELL_FAILED;
        ...
        ch->mana -= (pMobIndex->level[3]*4);
    }

    if ( IS_IMMORTAL(ch) || ( chance(ch, 75) && pMobIndex->level[3] - ch->level[best_magic_class(ch)] < 10 ) )
    { 
	mob = create_mobile( get_mob_index(MOB_VNUM_ANIMATED_CORPSE) );
	char_to_room( mob, ch->in_room );
	mob->level[max_sec_level(mob)] 	 = UMIN(ch->level[max_sec_level(ch)] / 2, pMobIndex->level[3]);
	mob->race  	 = pMobIndex->race;	/* should be undead */

        /* Fix so mobs wont have 0 hps and crash mud - Scryn 2/20/96 */
  	if (!pMobIndex->hitnodice)
	  mob->max_hit      = pMobIndex->level[3] * 8 + number_range(
                              pMobIndex->level[3] * pMobIndex->level[3]/ 4,
                              pMobIndex->level[3] * pMobIndex->level[3] );        
	else
	...
	mob->max_hit	 = UMAX( URANGE( mob->max_hit / 4,
				   ...
				   ch->level[best_magic_class(ch)] * dice(20,10)), 1 );

       
	mob->hit       = mob->max_hit;
	mob->damroll   = ch->level[best_magic_class(ch)] / 8;
	mob->hitroll   = ch->level[best_magic_class(ch)] / 6;
...
ch_ret spell_possess( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    ||   IS_AFFECTED(ch, AFF_CHARM)
    ||   level < victim->level[best_magic_class(victim)] 
    ||  victim->desc
    ...
    af.type      = sn;
    af.duration  = 20 + (ch->level[best_magic_class(ch)] - victim->level[best_magic_class(victim)] ) / 2;
    af.location  = 0;
    af.modifier  = 0;
...
ch_ret spell_obj_inv( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	case SA_CREATE:
	  ...
	  switch( SPELL_POWER(skill) )		/* clone object */
	  {
	     ...
	     default:
	     case SP_NONE:
		if ( ch->level[max_sec_level(ch)] - obj->level < 10
		||   obj->cost > ch->level[max_sec_level(ch)] * get_curr_int(ch) * get_curr_wis(ch) )
		{
		   failed_casting( skill, ch, NULL, obj );
		   return rNONE;
		}
		break;
	     case SP_MINOR:
		if ( ch->level[best_magic_class(ch)] - obj->level < 20
		||   obj->cost > ch->level[best_magic_class(ch)] * get_curr_int(ch) / 5 )
		{
		   failed_casting( skill, ch, NULL, obj );
		   return rNONE;
		}
		break;
	     case SP_GREATER:
		if ( ch->level[best_magic_class(ch)] - obj->level < 5
		||   obj->cost > ch->level[best_magic_class(ch)] * 10 * get_curr_int(ch) * get_curr_wis(ch) )
		{
		   failed_casting( skill, ch, NULL, obj );
		   return rNONE;
		}
		break;
	     case SP_MAJOR:
		if ( ch->level[best_magic_class(ch)] - obj->level < 0
		||   obj->cost > ch->level[best_magic_class(ch)] * 50 * get_curr_int(ch) * get_curr_wis(ch) )
		{
		   failed_casting( skill, ch, NULL, obj );
		   return rNONE;
		}
...
ch_ret spell_create_mob( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    mob->level[max_sec_level(mob)]   = UMIN( lvl, skill->dice ? dice_parse(ch, level, skill->dice) : mob->level[max_sec_level(mob)] );
    mob->armor	 = interpolate( mob->level[max_sec_level(mob)], 100, -100 );

    mob->max_hit = mob->level[best_fighting_class(mob)] * 8 + number_range(
				mob->level[best_fighting_class(mob)] * mob->level[best_fighting_class(mob)] / 4,
				mob->level[best_fighting_class(mob)] * mob->level[best_fighting_class(mob)] );
    mob->hit	 = mob->max_hit;
    mob->gold	 = 0;
...
ch_ret spell_hand_of_chaos( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    level       = UMAX(35, level);
    level       = UMIN(45, level);
    dam 	= 1.5*(2*level*number_range( 1, 6 ))-25;
