...
int get_cost( CHAR_DATA *ch, CHAR_DATA *keeper, OBJ_DATA *obj, bool fBuy )
{
    ...
    if ( ch->gold > (ch->level[max_sec_level(ch)] * ch->level[max_sec_level(ch)] * 10000) )
	richcustomer = TRUE;
    else
	richcustomer = FALSE;
    ...
    if ( fBuy )
    {
	profitmod = 13 - get_curr_cha(ch) + (richcustomer ? 15 : 1)
		  + ((URANGE(5,ch->level[max_sec_level(ch)],LEVEL_AVATAR)-20)/2);
	cost = (int) (obj->cost
...
void do_buy( CHAR_DATA *ch, char *argument )
{
	...
	if ( ch->gold < pet->level[max_sec_level(pet)] * pet->level[max_sec_level(pet)]/2 )
	{
	    send_to_char( "You can't afford it.\n\r", ch );
	    return;
	}

	if ( ch->level[max_sec_level(ch)] < pet->level[max_sec_level(pet)] )
	{
	    send_to_char( "You're not ready for this pet.\n\r", ch );
	    return;
	}

	maxgold = pet->level[max_sec_level(ch)] * pet->level[max_sec_level(ch)]/2;
	ch->gold	-= maxgold;
...
*/
	...
	maxgold = keeper->level[max_sec_level(keeper)] * keeper->level[max_sec_level(keeper)] * 50000;
	...
	if ( obj->level > ch->level[max_sec_level(ch)] )
	{
                ...
                pet = pet->next_in_room )
        {
            if ( xIS_SET(pet->act, ACT_PET) && IS_NPC(pet) )
            {
                ...
                pager_printf( ch, "[%2d] %8d - %s\n\r",
                        pet->level[max_sec_level(pet)],
                        pet->level[max_sec_level(pet)] * pet->level[max_sec_level(pet)]/2,
                        pet->short_descr );
            }
        }
        if ( !found )
...
void do_repair( CHAR_DATA *ch, char *argument )
{
    ...
    maxgold = keeper->level[max_sec_level(keeper)] * keeper->level[max_sec_level(keeper)] * 100000;
