...
void advance_level( CHAR_DATA *ch, int class )
{
	...
	save_char_obj( ch );
    sprintf( buf, "the %s",
	title_table [class] [ch->level[class]] [ch->sex == SEX_FEMALE ? 1 : 0] );
    set_title( ch, buf );

    add_hp	= (con_app[get_curr_con(ch)].hitp + number_range(
		    class_table[class]->hp_min,
		    class_table[class]->hp_max ))/num_classes(ch);
    add_mana	= (class_table[class]->fMana
		    ? UMAX( 1, number_range(2,(2*get_curr_int(ch)+
		    get_curr_wis(ch))/8)/num_classes(ch)) 
		    : 0);
    add_move	= (number_range( 5, (get_curr_con(ch)+get_curr_dex(ch))/4 ))/num_classes(ch);
    add_prac	= UMAX( wis_app[get_curr_wis(ch)].practice/num_classes(ch), 1);
    ...
    if (ch->level[class] >=LEVEL_AVATAR)
      ch->exp[class] = 0;
    if ( ch->level[class] == LEVEL_AVATAR )
    {
	DESCRIPTOR_DATA *d;

	sprintf( buf, "%s has just achieved the rank of Initiate Avatar!", ch->name );
    ...
    if ( ch->level[class] < LEVEL_IMMORTAL )
    {
      if ( IS_VAMPIRE(ch) )
        sprintf( buf,
...
void gain_exp( CHAR_DATA *ch, int gain )
{
    ...
    modgain = UMIN(modgain,
    	exp_level(ch, ch->level[class]+2, class) - exp_level(ch, ch->level[class]+1,class));

    ch->exp[class] = UMAX( 0, ch->exp[class] + modgain );

    if (NOT_AUTHED(ch) && ch->exp[class] >= exp_level(ch, ch->level[class]+1, class))
    {
	send_to_char("You can not ascend to a higher level until you are authorized.\n\r", ch);
	ch->exp[class] = (exp_level(ch, (ch->level[class]+1), class) - 1);
	return;
    }

    while ( ch->level[class] < LEVEL_AVATAR && ch->exp[class] >= exp_level(ch, ch->level[class]+1, class))
    {
...
int hit_gain( CHAR_DATA *ch )
{
    ...
    if ( IS_NPC(ch) )
    {
	gain = UMAX( 1, UMIN( (get_curr_con(ch)/4), ch->level[max_sec_level(ch)] ) );
    }
    else
    {
	gain = UMAX( 1, UMIN( get_curr_con(ch)/7, UMAX( 1, ch->level[max_sec_level(ch)] )));
	...
	case POS_SLEEPING: gain *= 3.0;	break;
	case POS_RESTING:  gain *= 2; break;
	    ...
	    if ( ch->pcdata->condition[COND_BLOODTHIRST] >= (8 + ch->level[4]) )
                gain *= 2;
            if ( IS_OUTSIDE( ch ) )
  	    {
...
int move_gain( CHAR_DATA *ch )
{
    ...
    if ( IS_NPC(ch) )
    {
	gain = ch->level[best_fighting_class(ch)];
    }
    else
    {
	gain = UMIN( 2*get_curr_dex(ch), UMAX( 1, 2 * ch->level[best_fighting_class(ch)] ) );
...
void gain_condition( CHAR_DATA *ch, int iCond, int value )
{
	...
	{
	case COND_FULL:
          if ( ch->level[max_sec_level(ch)] < LEVEL_AVATAR && !xIS_SET( ch->class, CLASS_VAMPIRE ) )
          {
	...
	case COND_THIRST:
	  ...
	  break;
        ...
        case COND_BLOODTHIRST:
          if ( xIS_SET( ch->class, CLASS_VAMPIRE ) && ch->level[max_sec_level(ch)] < LEVEL_AVATAR )
          {
            set_char_color( AT_BLOOD, ch );
...
/* Clean up 'animated corpses' that are not charmed' - Scryn */
	...
	||   pexit->to_room->area == ch->in_room->area ) )
	{
	    ...
	    ||    ch->position < POS_STANDING || xIS_SET(ch->act, ACT_GUARD))
	        continue;
	}
...
{   
	...
	if ( !IS_NPC(ch) && ch->level[max_sec_level(ch)] < LEVEL_IMMORTAL )
	{
	    ...
	    if ( ++ch->timer >= 1200 ) 
	    {
		if ( ch->timer == 1200 && ch->in_room && max_sec_level(ch) < LEVEL_IMMORTAL )
		{
		/*
...
*/
	...
	if ( !char_died(ch) )
	{
		...
		for ( paf = ch->first_affect; paf; paf = paf_next )
		{
			...
			{
			    if ( (*skill->spell_fun)(paf->modifier, ch->level[max_sec_level(ch)], ch, ch) == rCHAR_DIED
			    ||   char_died(ch) )
			    {
				died = TRUE;
				break;
...
{   
    OBJ_DATA *obj, corpse, corpse_next;
    sh_int AT_TEMP;
...
/* Corpse decay (npc corpses decay at 8 times the rate of pc corpses) - Narn */
        ...
        {
          sh_int timerfrac = (int) (obj->timer / 8 + 1);
...
void char_check( void )
{
	...
	if ( IS_NPC( ch ) )
	{
	    ...
	    /* running mobs	-Thoric */
	    if ( xIS_SET(ch->act, ACT_RUNNING) && ch->position > POS_FIGHTING )
	    {
		if ( !xIS_SET(ch->act, ACT_SENTINEL) && !xIS_SET(ch->act, ACT_GUARD)
		&&   ch->hunting )
		{
		    WAIT_STATE( ch, 2 * PULSE_VIOLENCE );
		    hunt_victim( ch );
	...
	else
	{
		...
		if ( !IS_AFFECTED( ch, AFF_AQUA_BREATH ) )
		{
			...
			dam = number_range( ch->max_hit / 100, ch->max_hit / 80 );
			dam = UMAX( 1, dam );
			if ( number_bits(3) == 0 )
		...
		&& !ch->mount )
		{
			...
			if ( ch->level[max_sec_level(ch)] < LEVEL_IMMORTAL )
			{
			    ...
			    if ( ch->move > 0 )
			    {
				mov = number_range( ch->max_move / 200, ch->max_move / 50 );
				mov = UMAX( 1, mov );
...
void aggr_update( void )
{
   ...
   */
	...
	if ( char_died(wch)
	||   IS_NPC(wch)
	||   wch->level[max_sec_level(wch)] >= LEVEL_IMMORTAL
	||  !wch->in_room )
	    continue;
	...
	for ( ch = wch->in_room->first_person; ch; ch = ch_next )
	{
		...
		&&   vch->level[max_sec_level(ch)] < LEVEL_IMMORTAL
		...
		&& victim->hit >= victim->max_hit )
		{
		    ...
		    if ( !IS_AWAKE(victim)
		    ||   number_percent( )+5 < ch->level[skill_level(ch, gsn_backstab)] )
		    {
  ...
  for ( d = first_descriptor; d; d = d->next ) 
      ...
      if ( victim && IS_WAITING_FOR_AUTH(victim) )
         ...
         sprintf( log_buf, " %s@%s new %s %s", victim->name,
            ...
            class_table[max_sec_level(victim)]->who_name ); 
         log_string( log_buf ); 
         to_channel( log_buf, CHANNEL_AUTH, "Auth", 1);
         }
      }
...
void auction_update (void)
{
	...
	case 3 : /* SOLD! */
		...
		>     can_carry_w( auction->buyer ) )
		{
	        ...
	        pay = (int)auction->bet * 0.01;
		tax = (int)auction->bet * 0.001;
		...
		>     can_carry_w( auction->seller ) )
		{
		    ...
		    obj_to_char (auction->item,auction->seller);
		tax = (int)auction->item->cost * 0.0001;
		boost_economy( auction->seller->in_room->area, tax );
