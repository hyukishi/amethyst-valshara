...
void learn_from_success( CHAR_DATA *ch, int sn )
{
	...
	else
	{
	 ...
	 ch->exp[min_sec_level(ch)] = UMAX( 0, ch->exp[min_sec_level(ch)] + gain );
         while ( ch->level[max_sec_level(ch)] < LEVEL_AVATAR && ch->exp[min_sec_level(ch)] >= exp_level(ch, ch->level[max_sec_level(ch)]+1, max_sec_level(ch)))
         {
...
void do_bash( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch)
    &&   ch->level[skill_level(ch, gsn_bash)] < skill_table[gsn_bash]->skill_level[skill_level(ch, gsn_bash)] )
    {
	send_to_char(
	    "You better leave the martial arts to fighters.\n\r", ch );
	return;
  ...
  if ( can_use_skill(ch, (number_percent () + chance), gsn_bash ) )
	...
	global_retcode = damage( ch, victim, number_range( 1, ch->level[best_fighting_class(ch)] ), gsn_bash );
    }
    else
    {
