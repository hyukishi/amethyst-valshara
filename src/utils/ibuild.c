...
fill_in_obj_page(CHAR_DATA * ch, OBJ_INDEX_DATA * idx, char page)
{
	...
	case 'b':
		...
		obj_page_b_data[32].data = ((idx->item_type == ITEM_CORPSE) ? check : space);
...
fill_in_mob_page(CHAR_DATA * ch, MOB_INDEX_DATA * idx, char page)
{
	...
	default:
		...
		mob_page_a_data[18].data = (xIS_SET( idx->class, CLASS_MAGE ) ) ? check : space;
		mob_page_a_data[19].data = (xIS_SET( idx->class, CLASS_CLERIC ) ) ? check : space;
		mob_page_a_data[20].data = (xIS_SET( idx->class, CLASS_THIEF ) ) ? check : space;
		mob_page_a_data[21].data = (xIS_SET( idx->class, CLASS_WARRIOR ) ) ? check : space;
		mob_page_a_data[22].data = (xIS_SET( idx->class, CLASS_VAMPIRE ) ) ? check : space;
		mob_page_a_data[23].data = (xIS_SET( idx->class, CLASS_DRUID ) ) ? check : space;
		mob_page_a_data[24].data = (xIS_SET( idx->class, CLASS_RANGER ) ) ? check : space;
		mob_page_a_data[25].data = (xIS_SET( idx->class, 20 ) ) ? check : space;
		mob_page_a_data[26].data = (xIS_SET( idx->class, 21 ) ) ? check : space;
		mob_page_a_data[27].data = (xIS_SET( idx->class, 22 ) ) ? check : space;
		mob_page_a_data[28].data = (xIS_SET( idx->class, 23 ) ) ? check : space;
		mob_page_a_data[29].data = (xIS_SET( idx->class, 24 ) ) ? check : space;
		mob_page_a_data[30].data = (xIS_SET( idx->class, 25 ) ) ? check : space;
		break;
	}
