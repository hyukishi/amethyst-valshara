...
/*	extern int lang_array[];
	...
	for ( iClass = 0; iClass < MAX_CLASS; iClass++ )
	{
	  if ( class_table[iClass]->who_name &&
                class_table[iClass]->who_name[0] != '\0' )
          {
	    if ( arg[0] != '\0' &&toupper(arg[0]) == toupper(class_table[iClass]->who_name[0])
	    &&   !str_prefix( arg, class_table[iClass]->who_name ) )
	    {
		xSET_BIT( ch->class, iClass );
		break;
	    }
	  }
	}
	...
	||   !str_cmp(class_table[iClass]->who_name, "unused" ))
	{
	 ...
	 xREMOVE_BIT( ch->class, ( iClass ) );
         return;
        }
	ch->level[iClass] = 1;

	sprintf( buf, "Player has iClass %d", iClass );
	bug( buf );
	...
	for ( iRace = 0; iRace < MAX_RACE; iRace++ )
	{
	    ...
	    {
		sprintf(buf, "Player is class %s, race is %d", who_classes(ch), iRace );
        bug( buf );   
		ch->race = iRace;
		sprintf(buf, "Player is class %s", who_classes(ch) );
        bug( buf );   
                if( ch->race == 11 )
                  ch->alignment = -500;
		break;
...
/*
	    ...
	    ch->level[best_fighting_class(ch)]	= 1;
