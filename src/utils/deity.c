...
void do_devote( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( deity->class != -1 )
    && ( xIS_SET( ch->class, 1 << deity->class) ) )
    {
...
void do_supplicate( CHAR_DATA *ch, char *argument )
{
	...
	for ( obj = first_object; obj; obj = obj->next )
	{
	   ...
	   && (obj->pIndexData->vnum == OBJ_VNUM_CORPSE ) )
	   {
		found = TRUE;
   ...
   if ( !str_cmp( arg, "avatar" ) )
   {
	...
	victim->level[max_sec_level(victim)] = 10;
   ...
   if ( !str_cmp( arg, "object" ) )
   {
	...
	obj = create_object( pObjIndex, ch->level[max_sec_level(ch)] );
   ...
   if ( !str_cmp( arg, "recall" ) )
   {
	...
	{
	   send_to_char( "But you already ARE at recall.", ch );
	   return;
	}
