...
int xp_compute( CHAR_DATA *gch, CHAR_DATA *victim )
{
    ...
    xp	  = (get_exp_worth( victim )
    	  *  URANGE( 0, (victim->level[max_sec_level(victim)] - gchlev) + 10, 13 )) / 10;
    align = gch->alignment - victim->alignment;
...
{
  int i, attempt, loose, die, percent, charm, nmbr, f, panic, tries, badroom;
  bool found;
  char buf[MAX_STRING_LENGTH];
 ...
 if(!IS_NPC(ch) || xIS_SET(ch->act, ACT_POLYSELF)) {  /* This only works on PC types */
  ...
  if (!(ch->fighting)) {
    for(f=0; f<(ch->flee); f++) {
      for(i=0; i<6; i++) {
        ...
        if (!get_exit( ch->in_room, attempt) == NULL )
	{
  ...
  do {
    for(i=0; i<3; i++) {
	  ...
	  if (!IS_NPC(ch) && panic) {
	    if (!xIS_SET( ch->class, CLASS_BLACKBELT ) || !xIS_SET( ch->class, CLASS_WARRIOR ) )
	      gain_exp(ch, -loose);
	  }
