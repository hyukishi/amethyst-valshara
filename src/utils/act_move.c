...
ROOM_INDEX_DATA *generate_exit( ROOM_INDEX_DATA *in_room, EXIT_DATA **pexit )
{
    ...
    if ( in_room->vnum > 100000 )	/* room is virtual */
    {
	serial = in_room->vnum;
...
ch_ret move_char( CHAR_DATA *ch, EXIT_DATA *pexit, int fall )
{
    ...
    if ( ch->position == POS_FIGHTING || (IS_NPC(ch) && xIS_SET(ch->act, ACT_MOUNTED) ) )
      return retcode;
    ...
    {
	if ( ch->level[max_sec_level(ch)] < to_room->area->low_hard_range )
	{
	    set_char_color( AT_TELL, ch );
	    switch( to_room->area->low_hard_range - ch->level[max_sec_level(ch)] )
	    {
		case 1:
	...
	if ( ch->level[min_sec_level(ch)] > to_room->area->hi_hard_range )
	{
	    set_char_color( AT_TELL, ch );
...
*/
    ...
    &&  ch->in_room->area != to_room->area )
    {
	if ( ch->level[max_sec_level(ch)] < to_room->area->low_soft_range )
	{
	   set_char_color( AT_MAGIC, ch );
	   send_to_char("You feel uncomfortable being in this strange land...\n\r", ch);
	}
	else
	if ( ch->level[min_sec_level(ch)] > to_room->area->hi_soft_range )
	{
	   set_char_color( AT_MAGIC, ch );
...
/*	if ( !IS_NPC( ch )
	&& ch->level[best_fighting_class(ch)] < skill_table[gsn_bashdoor]->skill_level[ch->class] )
	{
	...
	if ( !char_died( ch ) )
	   for ( gch = ch->in_room->first_person; gch; gch = gch->next_in_room )
	   {
		 ...
		 && ( ch->level[max_sec_level(ch)] - gch->level[max_sec_level(ch)] <= 4 )
		 && number_bits( 2 ) == 0 )
...
{
    OBJ_DATA *obj = NULL;
    if(!(ch->position == POS_FIGHTING) && !(ch->position == POS_EVASIVE) && !(ch->position == POS_DEFENSIVE) && !(ch->position == POS_AGGRESSIVE) && !(ch->position == POS_BERSERK)&& !(ch->position == POS_MOUNTED))
    {
    ...
    else obj = ch->sitting_on;

    if (obj != NULL)                                                              
    {
	if ((!IS_SET(obj->item_type,ITEM_FURNITURE) && !IS_SET(obj->item_type,ITEM_FOUNTAIN) )
	...
	/*if (obj != NULL && ch->sitting_on != obj)
	{
	    act( AT_PURPLE, "There's no more room on $p.",ch,obj,NULL,TO_CHAR);
	    return;
	}*/
        ...
        if (obj == NULL)
            {
            	...
            	act( AT_PURPLE,"$n wakes and sits up.", ch, NULL, NULL, TO_ROOM );
            }
            else if (IS_SET(obj->value[2],SIT_AT))
            {
            	act( AT_PURPLE, "You wake and sit at $p.",ch,obj,NULL,TO_CHAR);
            	act( AT_PURPLE, "$n wakes and sits at $p.",ch,obj,NULL,TO_ROOM);
            }
            else if (IS_SET(obj->value[2],SIT_ON))
            {
            	act( AT_PURPLE, "You wake and sit on $p.",ch,obj,NULL,TO_CHAR);
            	act( AT_PURPLE, "$n wakes and sits at $p.",ch,obj,NULL,TO_ROOM);
            }
            else
            {
            	act( AT_PURPLE, "You wake and sit in $p.",ch,obj,NULL,TO_CHAR);
            	act( AT_PURPLE,"$n wakes and sits in $p.",ch,obj,NULL,TO_ROOM);
            }
    ...
    case POS_RESTING:
	    ...
	    {
		act( AT_PURPLE, "You sit at $p.",ch,obj,NULL,TO_CHAR);
		act( AT_PURPLE, "$n sits at $p.",ch,obj,NULL,TO_ROOM);
	    }
	    else if (IS_SET(obj->value[2],SIT_ON))
	    {
		act( AT_PURPLE, "You sit on $p.",ch,obj,NULL,TO_CHAR);
		act( AT_PURPLE, "$n sits on $p.",ch,obj,NULL,TO_ROOM);
	    }
	ch->position = POS_SITTING;
	break;
    ...
    case POS_STANDING:
		...
		send_to_char("You sit down.\n\r",ch);
    		act( AT_PURPLE, "$n sits down on the ground.",ch,NULL,NULL,TO_ROOM);      
 	    }
	    else if (IS_SET(obj->value[2],SIT_AT))
	    {	
		act( AT_PURPLE, "You sit down at $p.",ch,obj,NULL,TO_CHAR);
		act( AT_PURPLE, "$n sits down at $p.",ch,obj,NULL,TO_ROOM);
	    }
	    else if (IS_SET(obj->value[2],SIT_ON))
	    {
		act( AT_PURPLE, "You sit on $p.",ch,obj,NULL,TO_CHAR);
	    ...
	    {
		act( AT_PURPLE, "You sit down in $p.",ch,obj,NULL,TO_CHAR);
	 	act( AT_PURPLE, "$n sits down in $p.",ch,obj,NULL,TO_ROOM);
	    }
