...
int get_exp_worth( CHAR_DATA *ch )
{
    ...
    exp = ch->level[max_sec_level(ch)] * ch->level[max_sec_level(ch)] * ch->level[min_sec_level(ch)] * 5;
    exp += ch->max_hit;
    exp -= (ch->armor-50) * 2;
...
int can_carry_n( CHAR_DATA *ch )
{
    ...
    if ( get_eq_char(ch, WEAR_SHIELD) )
      ++penalty;
    return URANGE(5, (ch->level[max_sec_level(ch)]+15)/5 + get_curr_dex(ch)-13 - penalty, 20);
}
