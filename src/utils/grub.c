                    ...
                    bool nm_sw, bool ng_sw )
{
  ...
  {
     if ( get_trust(ch) < po->carried_by->level[max_sec_level(po->carried_by )] ) return FALSE;
  ...
  else if ( po->in_obj )                 /* it's in a container          */
  {
     ...
     while( pt->in_obj )
            pt=pt->in_obj;
     if ( pt->carried_by && get_trust(ch) < pt->carried_by->level[max_sec_level(pt->carried_by)] )
        return FALSE;
   ...
   for (cou = 0; cou < MAX_KEY_HASH; cou++)
   {
      if ( mob_index_hash[cou] )
         for (pm = mob_index_hash[cou]; pm; pm = pm->next)
         {
            ...
            &&   pm->race==race && xIS_SET( pm->class, class ) && dis_cou++ < dis_num )
