...
void make_scraps( OBJ_DATA *obj )
{
  ...
  ||    obj->item_type == ITEM_QUIVER    || obj->item_type == ITEM_CORPSE)
  &&    obj->first_content )
  {
    if ( ch && ch->in_room )
    {
...
void make_corpse( CHAR_DATA *ch, CHAR_DATA *killer )
{
	...
	name 		= ch->name;
	corpse		= create_object(get_obj_index(OBJ_VNUM_CORPSE), 0);
	corpse->timer	= 8;
        corpse->value[3] = 100; /* for do_slice */
    if ( IS_NPC(ch) )
        corpse->value[4] = ch->pIndexData->vnum;  /* for slice and animate makes errors */
...
void make_blood( CHAR_DATA *ch )
{
	...
	obj->value[1]   = number_range( 3, UMIN(5, ch->level[max_sec_level(ch)]) );
