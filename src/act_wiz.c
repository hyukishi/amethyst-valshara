...
/*
 * Next three functions display the relevent bug/typo/idea files
 *            Puck
 */
...
void do_idealist( CHAR_DATA *ch, char *argument )
{
    set_pager_color( AT_IMMORT, ch );
...
void do_wizhelp( CHAR_DATA *ch, char *argument )
{
     ...
     for ( curr_lvl = LEVEL_HERO; curr_lvl <= get_trust( ch );
           curr_lvl++)
     {
         send_to_pager( "\n\r\n\r", ch);
         col = 1;
...
void do_retire( CHAR_DATA *ch, char *argument )
{
    ...
    if ( victim->level[max_sec_level(ch)] < LEVEL_SAVIOR )
    {
...
{ 
  ...
  if ( victim->level[min_sec_level(victim)] != 1 )  
  { 
...
void do_rstat( CHAR_DATA *ch, char *argument )
{
	...
	case SECT_UNDERGROUND:		sect = "Underground";	break;
        ...
        case SECT_HIVE:                 sect = "Hive";          break;   
}
    ...
    ch_printf_color( ch, "&cNiteDesc:\n\r&w%s", location->nitedesc );
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
	...
	get_curr_lck(victim) );
    pager_printf_color( ch,
	"&cLevel   : &P%s              ", who_level( victim )  );
    if ( victim->race  < MAX_NPC_RACE  && victim->race  >= 0 )
      pager_printf_color( ch, "&cClass  : &w%-10s   &cRace      : &w%-2.2d/%-10s\n\r",
	  who_classes(victim),
	  victim->race,
    ...
    pager_printf_color( ch, "&cSpeaks: &w%d   &cSpeaking: &w%d   &cExperience: &w%s",
    	victim->speaks, victim->speaking, who_exp(victim) );
...
void do_gwhere( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !pmobs )
    {
        ...
        &&   can_see( ch, victim )
        &&   victim->level[max_sec_level(victim)] >= low && victim->level[min_sec_level(victim)] <= high )
        {
            found = TRUE;
            pager_printf_color( ch, "&c(&C%2d&c) &w%-12.12s   [%-5d - %-19.19s]   &c%-25.25s\n\r",
		victim->level[max_sec_level(victim)], victim->name, victim->in_room->vnum, victim->in_room->area->name, victim->in_room->name );
	    count++;
        }
    }
    else
...
void do_gfighting( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !pmobs && !phating && !phunting)
    {
        ...
        && ( victim = d->character ) != NULL && !IS_NPC( victim ) && victim->in_room
        &&   can_see( ch, victim )
        &&   victim->fighting && victim->level[max_sec_level(victim)] >= low && victim->level[min_sec_level(victim)] <= high )
        {
            found = TRUE;
            pager_printf_color( ch, "&w%-12.12s &C|%2d &wvs &C%2d| &w%-16.16s [%5d]  &c%-20.20s [%5d]\n\r",
		victim->name, victim->level[max_sec_level(victim)], victim->fighting->who->level[max_sec_level(victim->fighting->who)],
    ...
    else if ( !phating && !phunting)
    {
        ...
        if ( IS_NPC( victim )
        &&   victim->in_room && can_see( ch, victim )
	&&   victim->fighting && victim->level[max_sec_level(victim)] >= low && victim->level[max_sec_level(victim)] <= high )
	{
            found = TRUE;
            pager_printf_color( ch, "&w%-12.12s &C|%2d &wvs &C%2d| &w%-16.16s [%5d]  &c%-20.20s [%5d]\n\r",
                victim->name, victim->level[max_sec_level(victim)], victim->fighting->who->level[max_sec_level(victim->fighting->who)],
    ...
    else if ( !phunting && phating)
    {
        ...
        if ( IS_NPC( victim )
        &&   victim->in_room && can_see( ch, victim )
	&&   victim->hating && victim->level[max_sec_level(victim)] >= low && victim->level[min_sec_level(victim)] <= high )
	{
            found = TRUE;
            pager_printf_color( ch, "&w%-12.12s &C|%2d &wvs &C%2d| &w%-16.16s [%5d]  &c%-20.20s [%5d]\n\r",
                victim->name, victim->level[max_sec_level(victim)], victim->hating->who->level[max_sec_level(victim->hating->who)], IS_NPC( victim->hating->who ) ?
    ...
    else if ( phunting )
    {
        ...
        if ( IS_NPC( victim )
        &&   victim->in_room && can_see( ch, victim )
	&&   victim->hunting && victim->level[max_sec_level(victim)] >= low && victim->level[max_sec_level(victim)] <= high )
	{
            found = TRUE;
            pager_printf_color( ch, "&w%-12.12s &C|%2d &wvs &C%2d| &w%-16.16s [%5d]  &c%-20.20s [%5d]\n\r",
                victim->name, victim->level[max_sec_level(victim)], victim->hunting->who->level[max_sec_level(victim->hunting->who)], IS_NPC( victim->hunting->who ) ?
...
void do_switch( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(victim) && ch->level[max_sec_level(ch)] < LEVEL_GREATER )
    {
...
void do_minvoke( CHAR_DATA *ch, char *argument )
{
	...
	pMobIndex->short_descr, pMobIndex->vnum, pMobIndex->player_name, victim->level[max_sec_level(victim)] );
    return;
}
...
void do_balzhur( CHAR_DATA *ch, char *argument )
{
    ...
    if ( (victim != ch) && victim->level[max_sec_level(victim)] >= get_trust( ch ) ) {
	...
	victim->level[max_sec_level(victim)]    = 2;
	victim->trust	 = 0;
	victim->exp[max_sec_level(victim)]      = 2000;
	victim->max_hit  = 10;
	victim->max_mana = 100;
	victim->max_move = 100;
    ...
    name = capitalize(victim->pcdata->filename);
    ...
    make_wizlist();
    advance_level( victim, max_sec_level(victim) );
...
void do_advance( CHAR_DATA *ch, char *argument )
{
    ...
    argument = one_argument( argument, arg2 );
    argument = one_argument( argument, arg3 );
    if ( arg1[0] == '\0' || arg2[0] == '\0' || arg3[0] == '\0' || !is_number( arg3 ) || !is_number( arg2 ) ) {
        send_to_char( "Syntax:  advance <character> <level> <class>\n\r", ch );
        return;
    }
  ...
  /*You can demote yourself but not someone else at your own trust.-- Narn*/
    ...
    if ( ( level = atoi( arg2 ) ) < 1 || level > MAX_LEVEL ) {
        send_to_char( "Level range is 1 to 115.\n\r", ch );
        return;
    }
    if ( ( class = atoi( arg3 ) ) < 0 || class > MAX_CLASS ) {
        ...
        else
	{
	  ...
	  send_to_char( "Deja vu!  Your mind reels as you re-live your past levels!\n\r", victim );
        ...
        victim->level[class]    = 1;
        victim->exp[class]      = exp_level(victim, 1, class);
        victim->max_hit  = 20;
        victim->max_mana = 100;
        victim->max_move = 100;
        for ( sn = 0; sn < top_sn; sn++ )
     ...
     /* Stuff added to make sure character's wizinvis level doesn't stay
        ...
        if ( xIS_SET(victim->act, PLR_WIZINVIS )
        && ( victim->level[class] <= LEVEL_AVATAR ) ) {
    ...
    else
    {
        ...
        if (victim->level[max_sec_level(victim)] >= LEVEL_AVATAR)
        {
          set_char_color( AT_IMMORT, victim );
    ...
    for ( iLevel = victim->level[class] ; iLevel < level; iLevel++ )
    {
    ...
    victim->exp[class]   = exp_level( victim, victim->level[class], class );
    victim->trust = 0;
    return;
}
...
void do_elevate( CHAR_DATA *ch, char *argument )
{
    ...
    if( victim->level[max_sec_level(victim)] == LEVEL_IMMORTAL ) {
       ...
       set_char_color( AT_LBLUE, victim );
       do_help(victim, "M_GODLVL2_" );
       victim->level[max_sec_level(victim)] = LEVEL_ACOLYTE;
       set_char_color( AT_WHITE, victim );
       advance_level( victim, max_sec_level(victim) );
       victim->exp[max_sec_level(victim)]   = exp_level( victim, victim->level[max_sec_level(victim)], max_sec_level(victim) );
       victim->trust = 0;
       return;
    }
    if( victim->level[max_sec_level(victim)] == LEVEL_ACOLYTE ) {
       send_to_char( "Elevating a player...\n\r", ch );
       set_char_color( AT_IMMORT, victim );
       act( AT_IMMORT, "$n begins to chant softly... then makes some arcane gestures...", ch, NULL, NULL, TO_ROOM );
       set_char_color( AT_WHITE, victim );
       ...
       set_char_color( AT_LBLUE, victim );
       do_help(victim, "M_GODLVL3_" ); 
       victim->level[max_sec_level(victim)] = LEVEL_CREATOR;  
       set_char_color( AT_WHITE, victim );
       advance_level( victim, max_sec_level(victim) );
       victim->exp[max_sec_level(victim)]   = exp_level( victim, victim->level[max_sec_level(victim)], max_sec_level(victim) );
       victim->trust = 0;
...
void do_restore( CHAR_DATA *ch, char *argument )
{
	...
	CHAR_DATA *vch_next;
        ...
        if ( !ch->pcdata && ch->level[max_sec_level(ch)] < LEVEL_SUB_IMPLEM )
          return;
	...
	for ( vch = first_char; vch; vch = vch_next )
	{
		...
		vch->pcdata->condition[COND_BLOODTHIRST] = (10 + vch->level[4]);
		update_pos (vch);
    ...
    victim->move = victim->max_move;
    if ( victim->pcdata )
      victim->pcdata->condition[COND_BLOODTHIRST] = (10 + victim->level[4]);
    update_pos( victim );
    if ( ch != victim )
...
void do_notitle( CHAR_DATA *ch, char *argument )
{
    ...
    else
    {
        ...
        sprintf( buf, "the %s",
                title_table [0] [victim->level[max_sec_level(victim)]] 
                [victim->sex == SEX_FEMALE ? 1 : 0] ); /* fix for multiclassing, Kyorlin*/
...
void do_invis( CHAR_DATA *ch, char *argument )
{
    ...
    {
	if ( ch->mobinvis < 2 )
	  ch->mobinvis = ch->level[max_sec_level(ch)];
    }
...
void do_mortalize( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "%s%c/%s", PLAYER_DIR, tolower(fname[0]),
                        capitalize( fname ) );
    if ( stat( buf, &fst ) != -1 )
    {
        ...
        DISPOSE( d->outbuf );
        DISPOSE( d );
        victim->level[max_sec_level(victim)]    = 50;
        victim->exp[max_sec_level(victim)]      = exp_level(victim, 50, max_sec_level(victim) );
        victim->max_hit  = 800;
        victim->max_mana = 800;
        victim->max_move = 800;
        for ( sn = 0; sn < top_sn; sn++ )
        ...
        if ( xIS_SET(victim->act, PLR_WIZINVIS) )
          victim->pcdata->wizinvis = victim->trust;
        if ( xIS_SET(victim->act, PLR_WIZINVIS)
                && (victim->level[max_sec_level(victim)] <= LEVEL_AVATAR))
        {
          xREMOVE_BIT(victim->act, PLR_WIZINVIS);
          victim->pcdata->wizinvis = victim->trust;
        }
    sprintf( buf, "%s%s", GOD_DIR, capitalize(victim->pcdata->filename) );
...
void do_newbieset( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( ( victim->level[max_sec_level(victim)] < 1 ) || ( victim->level[min_sec_level(victim)] > 5 ) ) && ( ch->level[max_sec_level(ch)] < LEVEL_GOD ) )
    {
...
void do_destroy( CHAR_DATA *ch, char *argument )
{
  ...
  name = capitalize_name( arg );
  strcpy( filekey, player_filename(arg) );
  sprintf( buf, "%s%c/%s", PLAYER_DIR, tolower(filekey[0]), capitalize(filekey) );
  sprintf( buf2, "%s%c/%s", BACKUP_DIR, tolower(filekey[0]), capitalize(filekey) );
  if ( !rename( buf, buf2 ) )
  {
    AREA_DATA *pArea;
...
void do_for (CHAR_DATA *ch, char *argument)
{
	...
	{ 
		...
		/*	p_next = p->next; */ /* In case someone DOES try to AT MOBS SLAY # */
			...
			if (IS_NPC(p) && fMobs)
				found = TRUE;
			else if (!IS_NPC(p) && p->level[max_sec_level(p)] >= LEVEL_IMMORTAL && fGods)
				found = TRUE;
			else if (!IS_NPC(p) && p->level[max_sec_level(p)] < LEVEL_IMMORTAL && fMortals)
				found = TRUE;
...
void do_orange( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char( "Syntax: orange <start> <end> <command> <arguments>\n\r", ch );
	send_to_char( "<command> being either oset, oinvoke, or ostat.", ch);
	return;
    }

    Start = atoi( arg1 );	End = atoi( arg2 );
    if ( Start < 1 || End < Start || Start > End || Start == End || End > 1000000 )
    {
	send_to_char( "Invalid range.\n\r", ch );
	return;
    }
    ...
    original = ch->in_room;
    char_to_room(ch, get_room_index( ROOM_VNUM_INVOKING ) );
    for( vnum = Start; vnum <= End; vnum++ )
    {
	if(!(get_obj_index(vnum) == NULL ) )
	create_object(get_obj_index(vnum),ch->level[max_sec_level(ch)] );
    }
    char_to_room(ch, original);
}
...
void do_qflag( CHAR_DATA *ch, char *argument )
{
    ...
    {
     	send_to_char( "There is no point in questflagging an immortal.\n\r", ch );
    	return;
    }
    if( IS_SET(victim->pcdata->flags, PCFLAG_QUEST ) )
    {
    	REMOVE_BIT(victim->pcdata->flags, PCFLAG_QUEST );
    	send_to_char( "Quest flag removed", ch);
    	send_to_char( "Your quest flag has been removed", ch);
    }
    else
    {
     	SET_BIT(victim->pcdata->flags, PCFLAG_QUEST );
    	send_to_char( "Quest mode enabled", ch);
    	send_to_char( "You have entered Quest Mode", victim);
    }
return;
}
...
void do_pcrename( CHAR_DATA *ch, char *argument )
{
    ...
    char arg1[MAX_INPUT_LENGTH];
    char arg2[MAX_INPUT_LENGTH];
    char dispname[MAX_INPUT_LENGTH];
    char filekey[MAX_INPUT_LENGTH];
    ...
    strcpy( dispname, capitalize_name(arg2) );
    strcpy( filekey, player_filename(dispname) );

    if  (!check_parse_name( dispname, 1) )
    {
    ...
    sprintf( newname, "%s%c/%s", PLAYER_DIR, tolower(filekey[0]),
                                 capitalize( filekey ) );
    ...
    if ( ch->pcdata->area )
    {
       ...
       sprintf( newfilename, "%s%s.are", BUILD_DIR, dispname);
       rename(filename, newfilename);
       sprintf( filename, "%s%s.are.bak", BUILD_DIR, victim->name);
       sprintf( newfilename, "%s%s.are.bak", BUILD_DIR, dispname);
       rename(filename, newfilename);
    }
       
    STRFREE( victim->name );
    victim->name = STRALLOC( dispname );    
    STRFREE( victim->pcdata->filename );
    victim->pcdata->filename = STRALLOC( filekey );
    remove( backname );
    if ( remove( oldname ) )
    {
