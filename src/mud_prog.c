...
/* Changed below from 1 to 0 for == 0 Shaddai */
  ...
  if ( chkchar )
  {
    ...
    {
      return mprog_veval(chkchar->level[max_sec_level(chkchar)], opr, atoi(rval), mob);
    }
    if ( !str_cmp(chck, "goldamt") )
    {
      return mprog_veval(chkchar->gold, opr, atoi(rval), mob);
    }
    if ( !str_cmp(chck, "class") ) /* fix this multiclassing kyorlin */
    {
      for(i=0;i<MAX_CLASS;i++)
      {
          if(xIS_SET(chkchar->class, i))
             if((char *)class_table[i]->who_name==rval)
                  return mprog_seval((char *)class_table[i]->who_name, opr, rval, mob);
      }
      return mprog_seval((char *)class_table[max_sec_level(chkchar)]->who_name, opr,
          rval, mob);
    }
	...
	&&    !ch->level[max_sec_level(ch)] > LEVEL_SAVIOR )
          lhsvl++;
      rhsvl = atoi(rval);
      if ( rhsvl < 0 ) rhsvl = 0;
      if ( !*opr ) strcpy( opr, "==" );
...
void mprog_entry_trigger( CHAR_DATA *mob, EXIT_DATA *pexit )
{
    if ( IS_NPC(mob)
