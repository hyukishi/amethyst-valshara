...
void do_fill( CHAR_DATA *ch, char *argument )
{
	...
	case ITEM_PIPE:
	  src_item1 = ITEM_HERB;	src_item2 = ITEM_HERB_CON;	break;
	case ITEM_CONTAINER:
	  src_item1 = ITEM_CONTAINER;	src_item2 = ITEM_CORPSE;
	  break;
    }
...
void do_drink( CHAR_DATA *ch, char *argument )
{
	...
	if ( IS_VAMPIRE(ch) && !IS_NPC(ch) )
	{
	    ...
	    &&   ch->level[4] > 5
	    &&   ch->pcdata->condition[COND_BLOODTHIRST] > (5+ch->level[4]/10) )
	    {
	    ...
	    if ( ch->pcdata->condition[COND_BLOODTHIRST] < (10 + ch->level[4]) )
	    {
	...
	if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
	{
	   act( AT_ACTION, "$n drinks from $p.", ch, obj, NULL, TO_ROOM );
           act( AT_ACTION, "You drink from $p.", ch, obj, NULL, TO_CHAR ); 
	}
...
/* Finally blood filled drinkcon will restore bp -XahXhaX */
        if ( obj->value[2] == 13 )
        {
            gain_condition(ch, COND_BLOODTHIRST, 1);
        }
...
void do_quaff( CHAR_DATA *ch, char *argument )
{
    ...
    &&   ch->pcdata->condition[COND_THIRST] >= 48  )
    {
	send_to_char( "You feel far to bloated to drink anything.\n\r", ch );
	return;
    }
...
void do_recite( CHAR_DATA *ch, char *argument )
{
    ...
    if( ( scroll->pIndexData->vnum == OBJ_VNUM_SCROLL_SCRIBING)
      &&(ch->level[max_sec_level(ch)] + 10 < scroll->value[0]))
    {
...
void DO_play( CHAR_DATA *ch, char *argument )
{
  ...
  if (arg == NULL)
  {
    send_to_char("You attempt to whistle out a few notes.\n\r", ch);
    return;
  }
  ...
  {
    send_to_char("There is no way you could play a song on that!\n\r", ch);
    return;
  }

  if (arg1 == NULL)
  {
    send_to_char("You cannot seem to remember what song you wanted to play.\n\r", ch);
    return;
  }

  act( AT_ACTION, "You play your tune on $p...", ch, obj, NULL, TO_CHAR );

  if ( (obj->value[0] == 99) && str_cmp( arg1, "A" ) && str_cmp( arg2, "D" ) && str_cmp( arg3, "F" ) && str_cmp( arg4, "A" ) && str_cmp( arg5, "D" ) && str_cmp( arg6, "F" ) && (arg7 == NULL ) && ( arg8 == NULL )
  {
...
void do_smoke( CHAR_DATA *ch, char *argument )
{
	...
	if ( IS_VALID_HERB( pipe->value[2] ) && pipe->value[2] < top_herb )
	{
	    ...
	    if ( skill->spell_fun )
		obj_cast_spell( sn, UMIN(skill->min_level, ch->level[max_sec_level(ch)]),
			ch, ch, NULL );
