...
void get_obj( CHAR_DATA *ch, OBJ_DATA *obj, OBJ_DATA *container )
{
    ...
    if ( !CAN_WEAR(obj, ITEM_TAKE)
       && (ch->level[max_sec_level(ch)] < sysdata.level_getobjnotake )  )
    {
...
void do_get( CHAR_DATA *ch, char *argument )
{
	...
	if ( number <= 1 && str_cmp( arg1, "all" ) && str_prefix( "all.", arg1 ) )
	{
	     ...
	     * --Shaddai
             */
	    if ( container->item_type == ITEM_CORPSE )
...
void wear_obj( CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace, sh_int wear_bit )
{
    ...
    separate_obj( obj );
    if ( get_trust( ch ) < obj->level && !(xIS_SET(ch->class, CLASS_BARBARIAN)) )
    {
    ...
    &&  (( IS_OBJ_STAT(obj, ITEM_ANTI_WARRIOR)
        && xIS_SET( ch->class, CLASS_WARRIOR )				)
    ||   ( IS_OBJ_STAT(obj, ITEM_ANTI_MAGE)
	&& xIS_SET( ch->class, CLASS_MAGE )				)
...
void do_sacrifice( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char( "That is not an acceptable sacrifice.", ch );
	return;
    }
    ...
    if ( IS_SET( obj->magic_flags, ITEM_PKDISARMED ) )
    {
        ...
        {
            if ( ch->level[min_sec_level(ch)] - obj->value[5] > 5
            ||   obj->value[5] - ch->level[max_sec_level(ch)] > 20 )
            {
...
void save_clan_storeroom( CHAR_DATA *ch, CLAN_DATA *clan )
{
    ...
    {
	templvl = ch->level[max_sec_level(ch)];
	ch->level[max_sec_level(ch)] = LEVEL_HERO;		/* make sure EQ doesn't get lost */
        contents = ch->in_room->last_content;
        if (contents)
	  fwrite_obj(ch, contents, fp, 0, OS_CARRY );
	fprintf( fp, "#END\n" );
	ch->level[max_sec_level(ch)] = templvl;
	fclose( fp );
	return;
    }
    return;
...
void do_auction (CHAR_DATA *ch, char *argument)
{
    ...
    if (IS_NPC(ch) && (str_cmp(arg1,"bid")) ) /* NPC can be extracted at any time and thus can't auction! */
	return;
...
/*		magic_bit_name( obj->magic_flags ), -- currently unused */
    ...
    if (!str_cmp(arg1,"bid") )
	    ...
	    if ( ch->level[max_sec_level(ch)] < auction->item->level )
	    {
...
*/		int dam = fall_count*obj->weight/2;
		...
		if ( obj->in_room->first_person && number_percent() > 15 )
		{
			...
			damage( vch, vch, dam*vch->level[max_sec_level(vch)], TYPE_UNDEFINED );
		}
    	/* Damage objects */
	    switch( obj->item_type )
     	    {
